...
from website.website.models import (
    Group,
    Server,
    User,
    add_group_to_server,
    ...
    if request.method == "POST":
       ...
       # password = request.form.get("password")
        ...
        existing_user = User.query.filter_by(
            username=username, server_id=server.id
        ).first()
        if existing_user:
...
def group_users(server_name, group_id):
    group_users = User.query.filter_by(
        server_id=Server.query.filter_by(name=server_name).first().id, group_id=group_id
    ).all()
...
@main_page.route(
    "/change_expiration/<int:user_id>/<server_name>", methods=["GET", "POST"]
)
def change_expiration(user_id, server_name):
    ...
    return render_template(
        "change_expiration.html", user_id=user_id, server_name=server_name
    )
...
def add_users_to_group(server_name, group_id):
    if request.method == "POST":
        ...
        group = Group.query.filter_by(
            id=group_id, server_id=Server.query.filter_by(name=server_name).first().id
        ).first()
        db.session.commit()
        if not group:
        ...
        for username in selected_users:
            user = User.query.filter_by(
                username=username, server_id=group.server_id
            ).first()
    ...
    users = get_users(server_name)
    return render_template(
        "add_user.html", server_name=server_name, group_id=group_id, users=users
    )
