...
class SchemaConst(Enum):
    RETAIN_SCHEMA = 1
    """Symbol indicating that a :class:`_schema.Table`, :class:`.Sequence` or in some
    cases a :class:`_schema.ForeignKey` object, in situations where the object is being
    copied for a :meth:`.Table.to_metadata` operation, should retain the schema name
    that it already has."""

    BLANK_SCHEMA = 2
    """
    Symbol indicating that a :class:`_schema.Table` or :class:`.Sequence` should have
    'None' for its schema, even if the parent :class:`_schema.MetaData` has specified a
    schema.
...
class SchemaItem(SchemaVisitable):
    ...
    def info(self) -> _InfoType:
        ...
        associated with this :class:`.SchemaItem`.
...
class HasConditionalDDL:
    """
    Define a class that includes the :meth:`.HasConditionalDDL.ddl_if` method, allowing
    for conditional rendering of DDL.
...
class Table(DialectKWArgs, HasSchemaAttr, TableClause, inspection.Inspectable["Table"]):
    ...
    """
    A collection of all :class:`_schema.Constraint` objects associated with this
    :class:`_schema.Table`.
    ...
    def _sentinel_column_characteristics(
        ...
        """
        Determine a candidate column (or columns, in case of a client generated
        composite primary key) which can be used as an "insert sentinel" for an INSERT
        statement.
    ...
    def autoincrement_column(self) -> Optional[Column[int]]:
        """
        Returns the :class:`.Column` object which currently represents the "auto
        increment" column, if any, else returns None.
    ...
    def add_is_dependent_on(self, table: Table) -> None:
        ...
        This is another Table object which must be created first before this one can, or
        dropped after this one.

        Usually, dependencies between tables are determined via ForeignKey objects.
    ...
    def create(self, bind: _CreateDropBind, checkfirst: bool = False) -> None:
        """
        Issue a ``CREATE`` statement for this :class:`_schema.Table`, using the given
    ...
    def drop(self, bind: _CreateDropBind, checkfirst: bool = False) -> None:
        """
        Issue a ``DROP`` statement for this :class:`_schema.Table`, using the given
        :class:`.Connection` or :class:`.Engine` for connectivity.
    ...
    def tometadata(
        ...
        """
        Return a copy of this :class:`_schema.Table` associated with a different
        :class:`_schema.MetaData`.
...
class Column(DialectKWArgs, SchemaItem, ColumnClause[_T]):
    ...
    """
    A collection of all :class:`_schema.ForeignKey` marker objects associated with this
    :class:`_schema.Column`.
    ...
    def _gen_static_annotations_cache_key(self) -> bool:  # type: ignore
        """
        Special attribute used by cache key gen, if true, we will use a static cache key
        for the annotations dictionary, else we will generate a new cache key for
        annotations each time.
    ...
    def _merge(self, other: Column[Any]) -> None:
        ...
        this is used by ORM pep-593 merge and will likely need a lot of fixes.
        """
...
def insert_sentinel(
    ...
    *,
    ...
    """
    Provides a surrogate :class:`_schema.Column` that will act as a dedicated insert
    :term:`sentinel` column, allowing efficient bulk inserts with deterministic
    RETURNING sorting for tables that don't otherwise have qualifying primary key
    configurations.
...
class ForeignKey(DialectKWArgs, SchemaItem):
    ...
    def _get_colspec(
        ...
        This is usually the equivalent of the string-based "tablename.colname" argument
    ...
    def references(self, table: Table) -> bool:
        """Return True if the given :class:`_schema.Table` is referenced by this
        :class:`_schema.ForeignKey`."""
    ...
    def get_referent(self, table: FromClause) -> Optional[Column[Any]]:
        """
        Return the :class:`_schema.Column` in the given :class:`_schema.Table` (or any
        :class:`.FromClause`) referenced by this :class:`_schema.ForeignKey`.
...
class DefaultGenerator(Executable, SchemaItem):
    ...
    This object is only present on column.default or column.onupdate. It's not valid as
    a server default.
    """
...
class _InsertSentinelColumnDefault(ColumnDefault):
    """
    Default generator that's specific to the use of a "sentinel" column when using the
...
class CallableColumnDefault(ColumnDefault):
    ...
    def _maybe_wrap_callable(
        ...
        This is to allow easy compatibility with default callables that aren't specific
        to accepting of a context.
        """
...
class ForeignKeyConstraint(ColumnCollectionConstraint):
    ...
    def referred_table(self) -> Table:
        ...
        This is a dynamically calculated attribute which may not be available if the
...
class MetaData(HasSchemaAttr):
    """
    A collection of :class:`_schema.Table` objects and their associated schema
    constructs.
    ...
    def sorted_tables(self) -> List[Table]:
        """
        Returns a list of :class:`_schema.Table` objects sorted in order of foreign key
        dependency.
...
class Identity(IdentityOptions, FetchedValue, SchemaItem):
    """
    Defines an identity column, i.e. "GENERATED { ALWAYS | BY DEFAULT } AS IDENTITY"
    syntax.
    ...
    def __init__(
        ...
        """
        Construct a GENERATED { ALWAYS | BY DEFAULT } AS IDENTITY DDL construct to
        accompany a :class:`_schema.Column`.
