...
def _generative(fn: _Fn) -> _Fn:
    ...
    This is basically the legacy decorator that copies the object and runs a method on
    the new copy.
    """
...
def _cloned_intersection(a: Iterable[_CLE], b: Iterable[_CLE]) -> Set[_CLE]:
    """
    Return the intersection of sets a and b, counting any overlap between 'cloned'
    predecessors.
...
class _DialectArgDict(MutableMapping[str, Any]):
    ...
    Maintains a separate collection of user-specified arguments and dialect-specified
    default arguments.
    """
...
class DialectKWArgs:
    """
    Establish the ability for a class to have dialect-specific arguments with defaults
    ...
    def dialect_kwargs(self):
        """
        A collection of keyword arguments specified as dialect-specific options to this
        construct.
...
class SchemaVisitor(ClauseVisitor):
    """Define the visiting for ``SchemaItem`` and more generally ``SchemaVisitable``
    objects."""
...
class ColumnCollection(Generic[_COLKEY, _COL_co]):
    """
    Collection of :class:`_expression.ColumnElement` instances, typically for
    ...
    def values(self) -> List[_COL_co]:
        """Return a sequence of :class:`_sql.ColumnClause` or :class:`_schema.Column`
        objects for all columns in this collection."""
        return [col for (_, col, _) in self._collection]

    def items(self) -> List[Tuple[_COLKEY, _COL_co]]:
        """Return a sequence of (key, column) tuples for all columns in this collection
        each consisting of a string key name and a :class:`_sql.ColumnClause` or
    ...
    def compare(self, other: ColumnCollection[Any, Any]) -> bool:
        """Compare this :class:`_expression.ColumnCollection` to another based on the
        names of the keys."""
    ...
    def get(
        ...
        """Get a :class:`_sql.ColumnClause` or :class:`_schema.Column` object based on a
        string key name from this :class:`_expression.ColumnCollection`."""
    ...
    def _init_proxy_index(self):
        ...
        proxy index is added in 2.0 to provide more efficient operation for the
        corresponding_column() method.

        For reasons of both time to construct new .c collections as well as memory
        conservation for large numbers of large .c collections, the proxy_index is only
        filled if corresponding_column() is called. once filled it stays that way, and
        new _ColumnMetrics objects created after that point will populate it with new
        data. Note this case would be unusual, if not nonexistent, as it means a .c
        collection is being mutated after corresponding_column() were used, however it
        is tested in test/base/test_utils.py.
        """
    ...
    def corresponding_column(
        ...
        :class:`_expression.ColumnCollection` which corresponds to that original
...
class DedupeColumnCollection(ColumnCollection[str, _NAMEDCOL]):
    ...
    def replace(
        ...
        """
        Add the given column to this collection, removing unaliased versions of this
        column  as well as existing columns with the same key.
...
def _entity_namespace(
    ...
    If not immediately available, does an iterate to find a sub-element that has one, if
    any.
    """
    try:
