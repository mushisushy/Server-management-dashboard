...
class Visitable:
    ...
    def _generate_compiler_dispatch(cls) -> None:
        ...
        def _compiler_dispatch(self: Visitable, visitor: Any, **kw: Any) -> str:
            """Look for an attribute named "visit_<visit_name>" on the visitor, and call
            it with the same kw params."""
            try:
                meth = getter(visitor)
...
class InternalTraversal(Enum):
    ...
    * :meth:`.HasCacheKey._gen_cache_key`
    ...
    """
    Visit a :class:`_expression.FromClause` object in the context of the ``columns``
    attribute.
    ...
    Examples include table and column names, bound parameter keys, special keywords such
    as "UNION", "UNION ALL".
    ...
    The keys of the dictionary should be strings, the values should be immutable and
    hashable.   The dictionary is considered to be significant for cache key generation.
    """
    ...
    """Visit a dictionary of string keys to values which may either be plain
    ...
    This is a dictionary of additional information about a ClauseElement that modifies
    its role.  It should be included when comparing or caching objects, however
    generating this key is relatively expensive.   Visitors should check the
    "_annotations" dict for non-None first before creating this key.
    """
    ...
    The value should be immutable and hashable, such as an integer. The value is
    ...
    """Visit the sequence represented by :class:`_expression.HasPrefixes` or
    :class:`_expression.HasSuffixes`."""
    ...
    """Visit the ``_statement_hints`` collection of a :class:`_expression.Select`
    object."""
    ...
    """Visit the values() ordered tuple list of an :class:`_expression.Update`
    object."""

    dp_dml_values = "DML_V"
    """Visit the values() dictionary of a :class:`.ValuesBase` (e.g. Insert or Update)
    object."""
    ...
    """
    Visit the propagate attrs dict.

    This hardcodes to the particular elements we care about right now.
    """
    ...
    This currently applies function call argument caching where some arguments should
    not be considered to be part of a cache key.
    """

    dp_inspectable = "IS"
    """Visit an inspectable object where the return value is a :class:`.HasCacheKey`
    object."""

    dp_multi = "M"
    """Visit an object that may be a :class:`.HasCacheKey` or may be a plain hashable
    object."""

    dp_multi_list = "MT"
    """Visit a tuple containing elements that may be :class:`.HasCacheKey` or may be a
    ...
    """Visit a list of inspectable objects which upon inspection are HasCacheKey
    objects."""


_TraverseInternalsType = List[Tuple[str, InternalTraversal]]
"""
A structure that defines how a HasTraverseInternals should be traversed.
...
class HasTraverseInternals:
    """
    Base for classes that have a "traverse internals" element, which defines all kinds
    of ways of traversing the elements of an object.
    ...
    def get_children(
        ...
        r"""
        Return immediate child :class:`.visitors.HasTraverseInternals` elements of this
        :class:`.visitors.HasTraverseInternals`.

        This is used for visit traversal.

        \**kw may contain flags that change the collection that is returned, for example
        to return a subset of items in order to cut down on larger traversals, or to
        return child items from a different context (such as schema-level collections
...
class HasTraversalDispatch:
    ...
    def dispatch(self, visit_symbol: InternalTraversal) -> Callable[..., Any]:
        """Given a method from :class:`.HasTraversalDispatch`, return the corresponding
        method on a subclass."""
...
class ExternallyTraversible(HasTraverseInternals, Visitable):
    ...
    def _copy_internals(self, *, omit_attrs: Tuple[str, ...] = (), **kw: Any) -> None:
        ...
        Called during a copy-and-traverse operation on newly shallow-copied elements to
        create a deep copy.

        The given clone function should be used, which may be applying additional
        transformations to the element (i.e. replacement traversal, cloned traversal,
        annotations).
        """
        raise NotImplementedError()
...
class ExternalTraversal(util.MemoizedSlots):
    ...
    :func:`.visitors.traverse` function.
    ...
    def iterate(
        ...
        """Traverse the given expression structure, returning an iterator of all
        elements."""
...
class CloningExternalTraversal(ExternalTraversal):
    ...
    :func:`.visitors.cloned_traverse` function.
    ...
    def copy_and_process(
        ...
        """Apply cloned traversal to the given list of elements, and return the new
        list."""
...
class ReplacingExternalTraversal(CloningExternalTraversal):
    """
    Base class for visitor objects which can traverse using the
    :func:`.visitors.replacement_traverse` function.
    ...
    def replace(self, elem: ExternallyTraversible) -> Optional[ExternallyTraversible]:
        ...
        If the method returns a new element, the element is used instead of creating a
        simple copy of the element.  Traversal will halt on the newly returned element
        if it is re-encountered.
        """
        return None
...
def cloned_traverse(
    ...
    """
    Clone the given expression structure, allowing modifications by visitors for mutable
    objects.
