...
class ClauseElement(
    ...
    if TYPE_CHECKING:
        ...
        def _propagate_attrs(self) -> _PropagateAttrsType:
            """Like annotations, however these propagate outwards liberally as SQL
            constructs are built, and are set up at construction time."""
            ...
    ...
    def _clone(self, **kw: Any) -> Self:
        ...
        This method may be used by a generative API.  Its also used as part of the
        "deep" copy afforded by a traversal that combines the _copy_internals() method.
        """
    ...
    def _negate_in_binary(self, negated_op, original_op):
        """
        A hook to allow the right side of a binary expression to respond to a negation
    ...
    def _with_binary_element_type(self, type_):
        """
        In the context of binary expression, convert the type of this object to the one
        given.
    ...
    def _constructor(self):
        ...
        This is for the purposes for creating a new object of this type.   Usually, its
        just the element's __class__. However, the "Annotated" version of the object
        overrides to return the class of its proxied element.
        """
    ...
    def _cloned_set(self):
        ...
        Includes this ClauseElement.  This accessor tends to be used for FromClause
        objects to identify 'equivalent' FROM clauses, regardless of transformative
        operations.
        """
        s = util.column_set()
    ...
    def _execute_on_scalar(
        ...
        """
        An additional hook for subclasses to provide a different implementation for
        connection.scalar() vs. connection.execute().
    ...
    def _get_embedded_bindparams(self) -> Sequence[BindParameter[Any]]:
        """
        Return the list of :class:`.BindParameter` objects embedded in the object.
    ...
    def compare(self, other: ClauseElement, **kw: Any) -> bool:
        r"""
        Compare this :class:`_expression.ClauseElement` to the given
...
class DQLDMLClauseElement(ClauseElement):
    """
    Represents a :class:`.ClauseElement` that compiles to a DQL or DML expression, not
    DDL.
...
class SQLColumnExpression(
    ...
    """
    A type that may be used to indicate any SQL column element or object that acts in
    place of one.
...
class ColumnElement(
    ...
    def _tq_label(self) -> Optional[str]:
        """
        The named label that can be used to target this column in a result set in a
        "table qualified" context.
    ...
    def _tq_key_label(self) -> Optional[str]:
        """
        A label-based version of 'key' that in some circumstances refers to this object
    ...
    """
    A flag used by select._columns_plus_names that helps to determine we are actually
    going to render in terms of "SELECT <col> AS <label>".

    This flag can be returned as False for some Column objects that want to be rendered
    as simple "SELECT <col>"; typically columns that don't have any parent table and are
    named the same as what the label would be in any case.
    """

    _allow_label_resolve = True
    """
    A flag that can be flipped to prevent a column from being resolvable by string label
    name.
    ...
    def proxy_set(self) -> FrozenSet[ColumnElement[Any]]:
        ...
        as of 2.0 this is explicitly deannotated columns.  previously it was effectively
        deannotated columns but wasn't enforced.  annotated columns should basically not
        go into sets if at all possible because their hashing behavior is very non-
        performant.
        """
    ...
    def _uncached_proxy_list(self) -> List[ColumnElement[Any]]:
        ...
        This list includes annotated columns which perform very poorly in set
        operations.
        """
    ...
    def shares_lineage(self, othercolumn: ColumnElement[Any]) -> bool:
        """Return True if the given :class:`_expression.ColumnElement` has a common
        ancestor to this :class:`_expression.ColumnElement`."""

        return bool(self.proxy_set.intersection(othercolumn.proxy_set))

    def _compare_name_for_result(self, other: ColumnElement[Any]) -> bool:
        """Return True if the given column element compares to this one when targeting
        within a result row."""
    ...
    def _expression_label(self) -> Optional[str]:
        """
        A suggested label to use in the case that the column has no name, which should
        be used if possible as the explicit 'AS <label>' where this expression would
        normally have an anon label.

        this is essentially mostly what _proxy_key does except it returns None if the
        column has a normal name that can be used.
        """
    ...
    def _make_proxy(
        ...
        **kw: Any,
        ...
        :class:`_expression.ColumnElement` as it appears in the select list of a
    ...
    def _dedupe_anon_label_idx(self, idx: int) -> str:
        """
        Label to apply to a column that is anon labeled, but repeated in the SELECT, so
        that we have to make an "extra anon" label that disambiguates it from the
        previous appearance.
...
class WrapsColumnExpression(ColumnElement[_T]):
    """
    Mixin that defines a :class:`_expression.ColumnElement` as a wrapper with special
...
class TextClause(
    ...
    def bindparams(
        ...
        **names_to_values: Any,
        ...
        :class:`_expression.TextClause` construct.
    ...
    def columns(
        ...
        **types: _TypeEngineArgument[Any],
        ...
        :class:`_expression.TextualSelect` object that serves the same role as a SELECT
        statement.
...
class ExpressionClauseList(OperatorExpression[_T]):
    """
    Describe a list of clauses, separated by an operator, in a column expression
    context.
...
class _label_reference(ColumnElement[_T]):
    ...
    This expression is any that includes an _order_by_label_element, which is a Label,
    or a DESC / ASC construct wrapping a Label.

    The production of _label_reference() should occur when an expression is added to
    this context; this includes the ORDER BY or GROUP BY of a SELECT statement, as well
    as a few other places, such as the ORDER BY within an OVER clause.
    """
...
class CollectionAggregate(UnaryExpression[_T]):
    ...
    The ANY and ALL keywords are available in different ways on different backends.  On
    PostgreSQL, they only work for an ARRAY type.  On MySQL, they only work for
    subqueries.
    """
...
class BinaryExpression(OperatorExpression[_T]):
    ...
    """Indicates that any database will know this is a boolean expression even if the
    database does not have an explicit boolean datatype."""
    ...
    def __bool__(self):
        """
        Implement Python-side "bool" for BinaryExpression as a simple "identity" check
        for the left and right attributes, if the operator is "eq" or "ne".  Otherwise
        the expression continues to not support "bool" like all other column
        expressions.
...
class Slice(ColumnElement[Any]):
    ...
    This is not a specific SQL construct at this level, but may be interpreted by
    specific dialects, e.g. PostgreSQL.
    """
...
class _OverrideBinds(Grouping[_T]):
    """
    Used by cache_key->_apply_params_to_element to allow compilation / execution of a
    SQL element that's been cached, using an alternate set of bound parameter values.

    This is used by the ORM to swap new parameter values into expressions that are
    embedded into loader options like with_expression(), selectinload().  Previously,
    this task was accomplished using the .params() method which would perform a deep-
    copy instead.  This deep copy proved to be too expensive for more complex
    expressions.
    ...
    def _gen_cache_key(
        ...
        """Generate a cache key for the given element, substituting its bind values for
        the translation values present."""
...
class Over(ColumnElement[_T]):
    ...
    This is a special operator against a so-called "window" function, as well as any
    aggregate function, which produces results relative to the result set itself.  Most
    modern SQL backends now support window functions.
    """
    ...
    element: ColumnElement[_T]
    """The underlying expression object to which this :class:`.Over` object refers."""
...
class NamedColumn(KeyedColumnElement[_T]):
    ...
    def _tq_label(self) -> Optional[str]:
        ...
        for table-bound columns this is <tablename>_<columnname>; all other expressions
        it resolves to .name.
        """
...
class ColumnClause(
    ...
    def _gen_tq_label(self, name: str, dedupe_on_key: bool = True) -> Optional[str]:
        ...
        used primarily for LABEL_STYLE_TABLENAME_PLUS_COL as well as the .columns
        collection on a Join object.
        """
        label: str
        t = self.table
        if self.is_literal:
...
class AnnotatedColumnElement(Annotated):
    ...
    def type(self):
        ...
        type is routinely changed on existing columns within the mapped_column()
        initialization process, and "type" is also consulted during the creation of SQL
        expressions.  Therefore it can change after it was already retrieved.  At the
        same time we don't want annotated objects having overhead when expressions are
        produced, so continue to memoize, but only when we have a non-null type.
        """
...
class _truncated_label(quoted_name):
    """A unicode subclass used to identify symbolic " "names that may require
    truncation."""
...
class conv(_truncated_label):
    """
    Mark a string indicating that a name has already been converted by a naming
    convention.
