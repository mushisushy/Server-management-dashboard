...
class _HasDialect(Protocol):
    """Protocol for Engine/Connection-like objects that have dialect attribute."""
...
"""Target for join() builds on _FromClauseArgument to include additional join target
roles such as those which come from the ORM."""

_OnClauseArgument = Union[_ColumnExpressionArgument[Any], roles.OnClauseRole]
"""Target for an ON clause, includes additional roles such as those which come from the
ORM."""
...
There's also edge cases like JSON expression assignment, which we would want the
DMLColumnRole to be able to accommodate.
"""
