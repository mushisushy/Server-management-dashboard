...
class _NoValueInList(Enum):
    NO_VALUE_IN_LIST = 0
    """Indicates we are trying to determine the type of an expression against an empty
    list."""
...
class TypeEngine(Visitable, Generic[_T]):
    ...
    """
    Render casts when rendering a value as an inline literal, e.g. with
    :meth:`.TypeEngine.literal_processor`.
    ...
    class Comparator(
        ...
        def _adapt_expression(
            ...
            """
            Evaluate the return type of <self> <op> <othertype>, and apply any
            adaptations to the given operator.
    ...
    """
    If True, the Python constant ``None`` is considered to be handled explicitly by this
    type.
    ...
    def evaluates_none(self) -> Self:
        """
        Return a copy of this type which has the :attr:`.should_evaluate_none` flag set
        to True.
    ...
    def _has_column_expression(self) -> bool:
        ...
        Allows the method to be skipped for the vast majority of expression types that
    ...
    def bind_expression(
        ...
        """
        Given a bind value (i.e. a :class:`.BindParameter` instance), return a SQL
    ...
    def with_variant(
        ...
        *dialect_names: str,
        ...
        r"""
        Produce a copy of this type object that will utilize the given type when applied
    ...
    def _resolve_for_literal(self, value: Any) -> Self:
        """
        Adjust this type given a literal Python value that will be stored in a bound
        parameter.
    ...
    def _resolve_for_python_type(
        ...
        """
        Given a Python type (e.g. ``int``, ``str``, etc. ) return an instance of this
        :class:`.TypeEngine` that's appropriate for this type.
    ...
    def _dialect_info(self, dialect: Dialect) -> _TypeMemoDict:
        """Return a dialect-specific registry which caches a dialect-specific
        implementation, bind processing function, and one or more result processing
        functions."""
    ...
    def adapt(
        ...
        This method is used internally to associate generic types with "implementation"
...
class TypeEngineMixin:
    """Classes which subclass this can act as "mixin" classes for TypeEngine."""
...
class Emulated(TypeEngineMixin):
    ...
    def adapt_to_emulated(
        ...
        **kw: Any,
        ...
        The impl should also be an "emulated" version of this type, most likely the same
        class as this type itself.
...
class TypeDecorator(SchemaEventTarget, ExternalType, TypeEngine[_T]):
    ...
    """
    Specify those Python types which should be coerced at the expression level to "IS
    <constant>" when compared using ``==`` (and same for ``IS NOT`` in conjunction with
    ``!=``).
    ...
    def _has_bind_processor(self) -> bool:
        ...
        Allows the base process_bind_param to raise NotImplementedError without needing
        to test an expensive exception throw.
        """
    ...
    def _has_result_processor(self) -> bool:
        ...
        Allows the base process_result_value to raise NotImplementedError without
        needing to test an expensive exception throw.
        """
    ...
    def bind_expression(
        ...
        """
        Given a bind value (i.e. a :class:`.BindParameter` instance), return a SQL
        expression which will typically wrap the given parameter.
...
class Variant(TypeDecorator[_T]):
    ...
    symbol is present for backwards-compatibility with workaround recipes, however this
    actual type should not be used.
    """
