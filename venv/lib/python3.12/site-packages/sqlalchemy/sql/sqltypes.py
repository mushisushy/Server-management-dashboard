...
class HasExpressionLookup(TypeEngineMixin):
    ...
    These rules are currently used by the numeric, integer and date types which have
    detailed cross-expression coercion rules.
    """
...
class Indexable(TypeEngineMixin):
    """A mixin that marks a type as supporting indexing operations, such as array or
    JSON structures."""
...
class Text(String):
    ...
    In SQL, usually corresponds to CLOB or TEXT.  In general, TEXT objects do not have a
    length; while some databases will accept a length argument here, it will be rejected
    by others.
    """
...
class Numeric(HasExpressionLookup, TypeEngine[_N]):
    """
    Base for non-integer numeric types, such as ``NUMERIC``, ``FLOAT``, ``DECIMAL``, and
    other variants.
...
class LargeBinary(_Binary):
    ...
    def __init__(self, length: Optional[int] = None):
        ...
        :param length: optional, a length for the column for use in DDL statements, for
...
class SchemaType(SchemaEventTarget, TypeEngineMixin):
    """
    Add capabilities to a type which allow for schema-level DDL to be associated with a
    type.
...
class Enum(String, SchemaType, Emulated, TypeEngine[Union[str, enum.Enum]]):
    ...
    def _enum_init(self, enums: _EnumTupleArg, kw: Dict[str, Any]) -> None:
        ...
        friendly init helper used by subclasses to remove all the Enum-specific keyword
        arguments from kw.  Allows all other arguments in kw to pass through.
        """
...
class Interval(Emulated, _AbstractInterval, TypeDecorator[dt.timedelta]):
    ...
    def __init__(
        ...
        :param native: when True, use the actual INTERVAL type provided by the database,
            if supported (currently PostgreSQL, Oracle Database). Otherwise, represent
            the interval data as an epoch value regardless.
        :param second_precision: For native interval types which support a "fractional
            seconds precision" parameter, i.e. Oracle Database and PostgreSQL
        :param day_precision: for native interval types which support a "day precision"
            parameter, i.e. Oracle Database.
        """
        super().__init__()
        self.native = native
...
class ARRAY(SchemaEventTarget, Indexable, Concatenable, TypeEngine[Sequence[_T]]):
    ...
    * When used with the Python ``getitem`` accessor, the number of dimensions
    ...
    zero_indexes = False
    """If True, Python zero-based indexes should be interpreted as one-based on the SQL
    expression side."""
    ...
    class Comparator(
        ...
        def contains(self, *arg: Any, **kw: Any) -> ColumnElement[bool]:
            """
            ``ARRAY.contains()`` not implemented for the base ARRAY type. Use the
    ...
    def _apply_item_processor(
        ...
        """
        Helper method that can be used by bind_processor(), literal_processor(), etc. to
        apply an item processor to elements of an array value, taking into account the
        'dimensions' for this array type.
...
class TIMESTAMP(DateTime):
    ...
    def __init__(self, timezone: bool = False):
        ...
        :param timezone: boolean. Indicates that the TIMESTAMP type should enable
            timezone support, if available on the target database. On a per-dialect
            basis is similar to "TIMESTAMP WITH TIMEZONE". If the target database does
            not support timezones, this flag is ignored.
        """
