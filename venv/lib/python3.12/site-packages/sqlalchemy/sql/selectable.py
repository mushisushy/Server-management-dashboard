...
"""The :class:`_expression.FromClause` class of SQL expression elements, representing
...
class ReturnsRows(roles.ReturnsRowsRole, DQLDMLClauseElement):
    """
    The base-most class for Core constructs that have some concept of columns that can
    represent rows.
    ...
    def _all_selected_columns(self) -> _SelectIterable:
        """
        A sequence of column expression objects that represents the "selected" columns
        of this :class:`_expression.ReturnsRows`.
    ...
    def is_derived_from(self, fromclause: Optional[FromClause]) -> bool:
        """
        Return ``True`` if this :class:`.ReturnsRows` is 'derived' from the given
        :class:`.FromClause`.
...
class Selectable(ReturnsRows):
    ...
    def replace_selectable(self, old: FromClause, alias: Alias) -> Self:
        """Replace all occurrences of :class:`_expression.FromClause` 'old' with the
        given :class:`_expression.Alias` object, returning a copy of this
    ...
    def corresponding_column(
        ...
        :attr:`_expression.Selectable.exported_columns` collection of this
        :class:`_expression.Selectable` which corresponds to that original
        :class:`_expression.ColumnElement` via a common ancestor column.
...
class HasHints:
    ...
    def with_statement_hint(self, text: str, dialect_name: str = "*") -> Self:
        """
        Add a statement hint to this :class:`_expression.Select` or other selectable
        object.
    ...
    def with_hint(
        ...
        r"""
        Add an indexing or other executional context hint for the given selectable to
        this :class:`_expression.Select` or other selectable object.
...
class FromClause(roles.AnonymizedFromClauseRole, Selectable):
    """
    Represent an element that can be used within the ``FROM`` clause of a ``SELECT``
    statement.
    ...
    * Methods to generate various derivations of a "from" clause, including
    ...
    def join(
        ...
        """
        Return a :class:`_expression.Join` from this :class:`_expression.FromClause` to
        another :class:`FromClause`.
    ...
    def outerjoin(
        ...
        """
        Return a :class:`_expression.Join` from this :class:`_expression.FromClause` to
        another :class:`FromClause`, with the "isouter" flag set to True.
    ...
    def is_derived_from(self, fromclause: Optional[FromClause]) -> bool:
        """
        Return ``True`` if this :class:`_expression.FromClause` is 'derived' from the
        given ``FromClause``.
    ...
    def _is_lexical_equivalent(self, other: FromClause) -> bool:
        """
        Return ``True`` if this :class:`_expression.FromClause` and the other represent
        the same lexical identity.

        This tests if either one is a copy of the other, or if they are the same via
        annotation identity.
        """
    ...
    def columns(
        ...
        """
        A named-based collection of :class:`_expression.ColumnElement` objects
    ...
    def primary_key(self) -> Iterable[NamedColumn[Any]]:
        """
        Return the iterable collection of :class:`_schema.Column` objects which comprise
        the primary key of this :class:`_selectable.FromClause`.
    ...
    def foreign_keys(self) -> Iterable[ForeignKey]:
        """
        Return the collection of :class:`_schema.ForeignKey` marker objects which this
    ...
    def _reset_column_collection(self) -> None:
        ...
        This collection is separate from all the other memoized things as it has shown
        to be sensitive to being cleared out in situations where enclosing code,
        typically in a replacement traversal scenario, has already established strong
        relationships with the exported columns.

        The collection is cleared for the case where a table is having a column added to
        it as well as within a Join during copy internals.
        """
    ...
    def _refresh_for_new_column(self, column: ColumnElement[Any]) -> None:
        """
        Given a column added to the .c collection of an underlying selectable, produce
        the local version of that column, assuming this selectable ultimately should
        proxy this column.

        this is used to "ping" a derived selectable to add a new column to its .c.
        collection when a Column has been added to one of the Table objects it
        ultimately derives from.

        If the given selectable hasn't populated its .c. collection yet, it should at
        least pass on the message to the contained selectables, but it will return None.

        This method is currently used by Declarative to allow Table columns to be added
        to a partially constructed inheritance mapping that may have already produced
        joins.  The method isn't public right now, as the full span of implications
        and/or caveats aren't yet clear.

        It's also possible that this functionality could be invoked by default via an
        event, which would require that selectables maintain a weak referencing
        collection of all derivations.
        """
...
class SelectLabelStyle(Enum):
    ...
    LABEL_STYLE_NONE = 0
    """
    Label style indicating no automatic labeling should be applied to the columns clause
    ...
    """
    Label style indicating all columns should be labeled as ``<tablename>_<columnname>``
    ...
    """
    Label style indicating that columns with a name that conflicts with an existing name
    should be labeled with a semi-anonymizing label when generating the columns clause
    of a SELECT statement.
...
class Join(roles.DMLTableRole, FromClause):
    """
    Represent a ``JOIN`` construct between two :class:`_expression.FromClause` elements.
...
class TableValuedAlias(LateralFromClause, Alias):
    ...
    def alias(self, name: Optional[str] = None, flat: bool = False) -> TableValuedAlias:
        ...
        This creates a distinct FROM object that will be distinguished from the original
    ...
    def lateral(self, name: Optional[str] = None) -> LateralFromClause:
        """
        Return a new :class:`_sql.TableValuedAlias` with the lateral flag set, so that
...
class CTE(
    ...
    def alias(self, name: Optional[str] = None, flat: bool = False) -> CTE:
        """
        Return an :class:`_expression.Alias` of this :class:`_expression.CTE`.
    ...
    def union(self, *other: _SelectStatementForCompoundArgument[Any]) -> CTE:
        r"""
        Return a new :class:`_expression.CTE` with a SQL ``UNION`` of the original CTE
    ...
    def union_all(self, *other: _SelectStatementForCompoundArgument[Any]) -> CTE:
        r"""
        Return a new :class:`_expression.CTE` with a SQL ``UNION ALL`` of the original
        CTE against the given selectables provided as positional arguments.
    ...
    def _get_reference_cte(self) -> CTE:
        ...
        Updated CTEs should still refer to the original CTE. This function returns this
...
class _ColumnsPlusNames(NamedTuple):
    ...
    """
    proxy_key that is to be part of the result map for this col.

    this is also the key in a fromclause.c or select.selected_columns collection
    """

    fallback_label_name: Optional[str]
    """Name that can be used to render an "AS <name>" when we have to render a label
    even though required_label_name was not given."""
...
class SelectsRows(ReturnsRows):
    """Sub-base of ReturnsRows for elements that deliver rows directly, namely SELECT
    ...
    def _generate_columns_plus_names(
        ...
        This is distinct from the _column_naming_convention generator that's intended
        for population of .c collections and similar, which has different rules.   the
        collection returned here calls upon the _column_naming_convention as well.
        """
...
class ForUpdateArg(ClauseElement):
    ...
    def __init__(
        ...
        *,
        ...
        """Represents arguments specified to :meth:`_expression.Select.for_update`."""
...
class Values(roles.InElementRole, Generative, LateralFromClause):
    ...
    def alias(self, name: Optional[str] = None, flat: bool = False) -> Self:
        """
        Return a new :class:`_expression.Values` construct that is a copy of this one
        with the given name.
    ...
    def lateral(self, name: Optional[str] = None) -> LateralFromClause:
        """
        Return a new :class:`_expression.Values` with the lateral flag set, so that it
        renders as LATERAL.
    ...
    def data(self, values: Sequence[Tuple[Any, ...]]) -> Self:
        """
        Return a new :class:`_expression.Values` construct, adding the given data to the
        data list.
    ...
    def scalar_values(self) -> ScalarValues:
        """
        Returns a scalar ``VALUES`` construct that can be used as a COLUMN element in a
        statement.
...
class ScalarValues(roles.InElementRole, GroupedElement, ColumnElement[Any]):
    """
    Represent a scalar ``VALUES`` construct that can be used as a COLUMN element in a
    statement.
...
class SelectBase(
    ...
    def _all_selected_columns(self) -> _SelectIterable:
        """
        A sequence of expressions that correspond to what is rendered in the columns
        clause, including :class:`_sql.TextClause` constructs.
    ...
    def exists(self) -> Exists:
        """
        Return an :class:`_sql.Exists` representation of this selectable, which can be
    ...
    def scalar_subquery(self) -> ScalarSelect[Any]:
        """
        Return a 'scalar' representation of this selectable, which can be used as a
        column expression.
    ...
    def label(self, name: Optional[str]) -> Label[Any]:
        """
        Return a 'scalar' representation of this selectable, embedded as a subquery with
        a label.
    ...
    def _ensure_disambiguated_names(self) -> Self:
        """Ensure that the names generated by this selectbase will be disambiguated in
...
class GenerativeSelect(DialectKWArgs, SelectBase, Generative):
    ...
    def _offset_or_limit_clause(
        ...
        This handles incoming integers and converts to an expression; if an expression
        is already given, it is passed through.
        """
    ...
    def _limit(self) -> Optional[int]:
        """
        Get an integer value for the limit.

        This should only be used by code that cannot support a limit as a BindParameter
        or other custom clause as it will throw an exception if the limit isn't
        currently set to an integer.
        """
        return self._offset_or_limit_clause_asint(self._limit_clause, "limit")

    def _simple_int_clause(self, clause: ClauseElement) -> bool:
        """True if the clause is a simple integer, False if it is not present or is a
        SQL expression."""
    ...
    def _offset(self) -> Optional[int]:
        """
        Get an integer value for the offset.

        This should only be used by code that cannot support an offset as a
        BindParameter or other custom clause as it will throw an exception if the offset
...
class SelectState(util.MemoizedSlots, CompileState):
    ...
    def _normalize_froms(
        ...
        """
        Given an iterable of things to select FROM, reduce them to what would actually
        render in the FROM clause of a SELECT.

        This does the job of checking for JOINs, tables, etc. that are in fact
        overlapping due to cloning, adaption, present in overlapping joins, etc.
        """
    ...
    def _get_display_froms(
        ...
        Takes into account a set of existing froms which may be rendered in the FROM
        clause of enclosing selects; this Select may want to leave those absent if it is
    ...
    def _join_determine_implicit_left_side(
        ...
        """When join conditions don't express the left side explicitly, determine if an
        existing FROM or entity in this query can serve as the left hand side."""
...
class _MemoizedSelectEntities(
    ...
    """
    Represents partial state from a Select object, for the case where Select.columns()
    has redefined the set of columns/entities the statement will be SELECTing from. This
    object represents the entities from the SELECT before that transformation was
    applied, so that transformations that were made in terms of the SELECT at that time,
    such as join() as well as options(), can access the correct context.
...
class Select(
    ...
    def filter_by(self, **kwargs: Any) -> Self:
        r"""Apply the given filtering criterion as a WHERE clause to this select."""
    ...
    def column_descriptions(self) -> Any:
        """
        Return a :term:`plugin-enabled` 'column descriptions' structure referring to the
        columns which are SELECTed by this statement.
    ...
    def from_statement(self, statement: roles.ReturnsRowsRole) -> ExecutableReturnsRows:
        """
        Apply the columns which this :class:`.Select` would select onto another
        statement.
    ...
    def join(
        ...
        *,
        ...
        r"""
        Create a SQL JOIN against this :class:`_expression.Select` object's criterion
    ...
    def outerjoin_from(
        ...
        *,
        ...
        r"""
        Create a SQL LEFT OUTER JOIN against this :class:`_expression.Select` object's
        criterion and apply generatively, returning the newly resulting
        :class:`_expression.Select`.
    ...
    def columns_clause_froms(self) -> List[FromClause]:
        """
        Return the set of :class:`_expression.FromClause` objects implied by the columns
    ...
    def inner_columns(self) -> _SelectIterable:
        """
        An iterator of all :class:`_expression.ColumnElement` expressions which would be
        rendered into the columns clause of the resulting SELECT statement.
    ...
    def add_columns(self, *entities: _ColumnsClauseArgument[Any]) -> Select[Any]:
        r"""
        Return a new :func:`_expression.select` construct with the given entities
        appended to its columns clause.
    ...
    def column(self, column: _ColumnsClauseArgument[Any]) -> Select[Any]:
        """
        Return a new :func:`_expression.select` construct with the given column
        expression added to its columns clause.
    ...
    def reduce_columns(self, only_synonyms: bool = True) -> Select[Any]:
        """
        Return a new :func:`_expression.select` construct with redundantly named,
    ...
    def with_only_columns(
        ...
        **__kw: Any,
        ...
        r"""
        Return a new :func:`_expression.select` construct with its columns clause
        replaced with the given entities.
    ...
    def whereclause(self) -> Optional[ColumnElement[Any]]:
        """
        Return the completed WHERE clause for this :class:`_expression.Select`
        statement.
    ...
    def where(self, *whereclause: _ColumnExpressionArgument[bool]) -> Self:
        """Return a new :func:`_expression.select` construct with the given expression
    ...
    def having(self, *having: _ColumnExpressionArgument[bool]) -> Self:
        """Return a new :func:`_expression.select` construct with the given expression
        added to its HAVING clause, joined to the existing clause via AND, if any."""
    ...
    def distinct(self, *expr: _ColumnExpressionArgument[Any]) -> Self:
        r"""
        Return a new :func:`_expression.select` construct which will apply DISTINCT to
        the SELECT statement overall.
    ...
    def select_from(self, *froms: _FromClauseArgument) -> Self:
        r"""
        Return a new :func:`_expression.select` construct with the given FROM
        expression(s) merged into its list of FROM objects.
    ...
    def correlate(
        ...
        *fromclauses: Union[Literal[None, False], _FromClauseArgument],
        ...
        r"""
        Return a new :class:`_expression.Select` which will correlate the given FROM
    ...
    def correlate_except(
        ...
        *fromclauses: Union[Literal[None, False], _FromClauseArgument],
        ...
        r"""
        Return a new :class:`_expression.Select` which will omit the given FROM clauses
        from the auto-correlation process.
    ...
    def _generate_fromclause_column_proxies(
        ...
        *,
        ...
        """Generate column proxies to place in the exported ``.c`` collection of a
        subquery."""
    ...
    def self_group(
        ...
        """
        Return a 'grouping' construct as per the :class:`_expression.ClauseElement`
        specification.

        This produces an element that can be embedded in an expression. Note that this
        method is called automatically as needed when constructing expressions and
        should not require explicit use.
        """
        if (
    ...
    def union(
        ...
        r"""
        Return a SQL ``UNION`` of this select() construct against the given selectables
    ...
    def union_all(
        ...
        selectables provided as positional arguments.
...
class ScalarSelect(roles.InElementRole, Generative, GroupedElement, ColumnElement[_T]):
    ...
    def where(self, crit: _ColumnExpressionArgument[bool]) -> Self:
        ...
        :class:`_expression.ScalarSelect`."""
    ...
    def correlate(
        ...
        *fromclauses: Union[Literal[None, False], _FromClauseArgument],
        ...
        r"""
        Return a new :class:`_expression.ScalarSelect` which will correlate the given
        FROM clauses to that of an enclosing :class:`_expression.Select`.
    ...
    def correlate_except(
        ...
        *fromclauses: Union[Literal[None, False], _FromClauseArgument],
        ...
        r"""
        Return a new :class:`_expression.ScalarSelect` which will omit the given FROM
...
class Exists(UnaryExpression[bool]):
    ...
    def select_from(self, *froms: _FromClauseArgument) -> Self:
        """
        Return a new :class:`_expression.Exists` construct, applying the given
        expression to the :meth:`_expression.Select.select_from` method of the select
        statement contained.
    ...
    def where(self, *clause: _ColumnExpressionArgument[bool]) -> Self:
        """
        Return a new :func:`_expression.exists` construct with the given expression
        added to its WHERE clause, joined to the existing clause via AND, if any.
...
class TextualSelect(SelectBase, ExecutableReturnsRows, Generative):
    ...
    :class:`_expression.SelectBase` interface.
