...
class FunctionElement(Executable, ColumnElement[_T], FromClause, Generative):
    ...
    def scalar_table_valued(
        ...
        :class:`_functions.FunctionElement` as a scalar table-valued expression.
    ...
    def clauses(self) -> ClauseList:
        """Return the underlying :class:`.ClauseList` which contains the arguments for
        this :class:`.FunctionElement`."""
    ...
    def within_group_type(
        ...
        """
        For types that define their return type as based on the criteria within a WITHIN
        GROUP (ORDER BY) expression, called by the :class:`.WithinGroup` construct.
    ...
    def select(self) -> Select[Tuple[_T]]:
        """
        Produce a :func:`_expression.select` construct against this
        :class:`.FunctionElement`.
    ...
    def entity_namespace(self) -> _EntityNamespace:
        """Overrides FromClause.entity_namespace as functions are generally column
        expressions and not FromClauses."""
...
class next_value(GenericFunction[int]):
    """
    Represent the 'next value', given a :class:`.Sequence` as its single argument.

    Compiles into the appropriate function on each backend, or will raise
    NotImplementedError if used on a backend that does not provide support for
    sequences.
    """
...
class ReturnTypeFromArgs(GenericFunction[_T]):
    """Define a function whose return type is bound to the type of its arguments."""
...
class OrderedSetAgg(GenericFunction[_T]):
    """Define a function where the return type is based on the sort expression type as
    defined by the expression passed to the :meth:`.FunctionElement.within_group`
    method."""
