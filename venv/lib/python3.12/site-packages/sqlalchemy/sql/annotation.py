...
"""
The :class:`.Annotated` class and related routines; creates hash-equivalent copies of
SQL constructs which contain context-specific markers and associations.
...
class SupportsWrappingAnnotations(SupportsAnnotations):
    ...
    def _annotate(self, values: _AnnotationDict) -> Self:
        """Return a copy of this ClauseElement with annotations updated by the given
        dictionary."""
        return Annotated._as_annotated_instance(self, values)  # type: ignore

    def _with_annotations(self, values: _AnnotationDict) -> Self:
        """Return a copy of this ClauseElement with annotations replaced by the given
        dictionary."""
        return Annotated._as_annotated_instance(self, values)  # type: ignore
    ...
    def _deannotate(
        ...
        """
        Return a copy of this :class:`_expression.ClauseElement` with annotations
        removed.

        :param values: optional tuple of individual values to remove.
        """
        if clone:
            s = self._clone()
            return s
...
def _deep_annotate(
    ...
    *,
    ...
    """
    Deep copy the given ClauseElement, annotating each element with the given
...
def _shallow_annotate(element: _SA, annotations: _AnnotationDict) -> _SA:
    """
    Annotate the given ClauseElement and copy its internals so that internal objects
    refer to the new annotated object.

    Basically used to apply a "don't traverse" annotation to a selectable, without
    digging throughout the whole structure wasting time.
    """
...
def _new_annotation_type(
    ...
    """Generates a new class that subclasses Annotated and proxies a given element
    type."""
