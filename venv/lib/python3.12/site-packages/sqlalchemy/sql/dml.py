...
class UpdateBase(
    ...
    def return_defaults(
        ...
        *cols: _DMLColumnArgument,
        ...
        """
        Make use of a :term:`RETURNING` clause for the purpose of fetching server-side
        expressions and defaults, for supporting backends only.
    ...
    def is_derived_from(self, fromclause: Optional[FromClause]) -> bool:
        ...
        Since these are DMLs, we dont want such statements ever being adapted so we
        return False for derives.
        """
        return False
    ...
    def entity_description(self) -> Dict[str, Any]:
        """
        Return a :term:`plugin-enabled` description of the table and/or entity which
        this DML construct is operating against.
    ...
    def returning_column_descriptions(self) -> List[Dict[str, Any]]:
        """
        Return a :term:`plugin-enabled` description of the columns which this DML
        construct is RETURNING against, in other words the expressions established as
        part of :meth:`.UpdateBase.returning`.
...
class ValuesBase(UpdateBase):
    ...
    """Used by extensions to Insert etc. to add additional syntacitcal constructs, e.g.
    ON CONFLICT etc."""
    ...
    def values(
        ...
        **kwargs: Any,
        ...
        r"""
        Specify a fixed VALUES clause for an INSERT statement, or the SET clause for an
        UPDATE.
...
class Insert(ValuesBase):
    ...
    def from_select(
        ...
        """
        Return a new :class:`_expression.Insert` construct which represents an
...
class ReturningInsert(Insert, TypedReturnsRows[_TP]):
    """
    Typing-only class that establishes a generic type form of :class:`.Insert` which
    tracks returned column types.
...
class DMLWhereBase:
    ...
    def where(self, *whereclause: _ColumnExpressionArgument[bool]) -> Self:
        """
        Return a new construct with the given expression(s) added to its WHERE clause,
...
class Update(DMLWhereBase, ValuesBase):
    ...
    def ordered_values(self, *args: Tuple[_DMLColumnArgument, Any]) -> Self:
        """
        Specify the VALUES clause of this UPDATE statement with an explicit parameter
        ordering that will be maintained in the SET clause of the resulting UPDATE
        statement.
...
class ReturningUpdate(Update, TypedReturnsRows[_TP]):
    """
    Typing-only class that establishes a generic type form of :class:`.Update` which
    tracks returned column types.
...
class ReturningDelete(Update, TypedReturnsRows[_TP]):
    """
    Typing-only class that establishes a generic type form of :class:`.Delete` which
    tracks returned column types.
