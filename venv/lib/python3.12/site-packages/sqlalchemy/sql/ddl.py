...
"""Provides the hierarchy of DDL-defining schema items as well as routines to invoke
them for a create/drop call."""
...
class BaseDDLElement(ClauseElement):
    """
    The root of DDL constructs, including those that are sub-elements within the "create
    table" and other processes.
...
class ExecutableDDLElement(roles.DDLRole, Executable, BaseDDLElement):
    ...
    def against(self, target: SchemaItem) -> Self:
        """
        Return a copy of this :class:`_schema.ExecutableDDLElement` which will include
        the given target.
    ...
    def execute_if(
        ...
        r"""
        Return a callable that will execute this :class:`_ddl.ExecutableDDLElement`
...
class _DropView(_DropBase["Table"]):
    ...
    Used by the test suite for dialect-agnostic drops of views. This object will
    eventually be part of a public "view" API.
    """
...
class CreateColumn(BaseDDLElement):
    """
    Represent a :class:`_schema.Column` as rendered in a CREATE TABLE statement, via the
...
class InvokeDDLBase(SchemaVisitor):
    ...
    def with_ddl_events(self, target, **kw):
        """Helper context manager that will apply appropriate DDL events to a CREATE or
        DROP operation."""
...
def sort_tables_and_constraints(
    ...
    """
    Sort a collection of :class:`_schema.Table`  / :class:`_schema.ForeignKeyConstraint`
    objects.
