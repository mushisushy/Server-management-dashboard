...
class Connection(ConnectionEventsTarget, inspection.Inspectable["Inspector"]):
    ...
    def schema_for_object(self, obj: HasSchemaAttr) -> Optional[str]:
        """Return the schema name for the given schema item taking into account current
    ...
    def invalidated(self) -> bool:
        ...
        This does not indicate whether or not the connection was invalidated at the pool
        level, however
        """
    ...
    def get_isolation_level(self) -> IsolationLevel:
        """
        Return the current **actual** isolation level that's present on the database
    ...
    def info(self) -> _InfoType:
        ...
        this :class:`_engine.Connection`, allowing user-defined data to be associated
        with the connection.
    ...
    def begin_nested(self) -> NestedTransaction:
        """
        Begin a nested transaction (i.e. SAVEPOINT) and return a transaction handle that
        controls the scope of the SAVEPOINT.
    ...
    def scalars(
        ...
        *,
        ...
        """
        Executes and returns a scalar result set, which yields scalar values from the
    ...
    def exec_driver_sql(
        ...
        r"""
        Executes a string SQL statement on the DBAPI cursor directly, without any SQL
        compilation steps.
    ...
    def _exec_single_context(
        ...
        """Continue the _execute_context() method for a single DBAPI cursor.execute() or
        cursor.executemany() call."""
    ...
    def _exec_insertmany_context(
        ...
        """Continue the _execute_context() method for an "insertmanyvalues" operation,
        which will invoke DBAPI cursor.execute() one or more times with individual log
        and event hook calls."""
    ...
    def _cursor_execute(
        ...
        This method is used by DefaultDialect for special-case executions, such as for
        sequences and column defaults. The path of statement execution in the majority
        of cases terminates at _execute_context().
        """
        if self._has_events or self.engine._has_events:
            for fn in self.dispatch.before_cursor_execute:
    ...
    def _run_ddl_visitor(
        ...
        **kwargs: Any,
        ...
        This method is only here so that the MockConnection can change the options given
        to the visitor so that "checkfirst" is skipped.
        """
        visitorcallable(
...
class Transaction(TransactionalContext):
    ...
    def _deactivated_from_connection(self) -> bool:
        """True if this transaction is totally deactivated from the connection and
        therefore can no longer affect its state."""
        raise NotImplementedError()
    ...
    def close(self) -> None:
        ...
        If this transaction is the base transaction in a begin/commit nesting, the
        transaction will rollback().  Otherwise, the method returns.

        This is used to cancel a Transaction without affecting the scope of an enclosing
        transaction.
        """
        try:
            self._do_close()
...
class Engine(
    ...
    :class:`~sqlalchemy.engine.interfaces.Dialect` together to provide a source of
    database connectivity and behavior.
    ...
    @property
    def name(self) -> str:
        """String name of the :class:`~sqlalchemy.engine.interfaces.Dialect` in use by
        this :class:`Engine`."""
    ...
    @property
    def driver(self) -> str:
        """Driver name of the :class:`~sqlalchemy.engine.interfaces.Dialect` in use by
        this :class:`Engine`."""
    ...
    def begin(self) -> Iterator[Connection]:
        """
        Return a context manager delivering a :class:`_engine.Connection` with a
