...
about a database connection specification.
...
class URL(NamedTuple):
    ...
    * :attr:`_engine.URL.query`: an immutable mapping representing the query
    ...
    """Password, which is normally a string but may also be any object that has a
    ``__str__()`` method."""
    ...
    May also be a data source name for some drivers.
    """
    ...
    def difference_update_query(self, names: Iterable[str]) -> URL:
        """
        Remove the given names from the :attr:`_engine.URL.query` dictionary, returning
        the new :class:`_engine.URL`.
    ...
    def normalized_query(self) -> Mapping[str, Sequence[str]]:
        """
        Return the :attr:`_engine.URL.query` dictionary with values normalized into
        sequences.
    ...
    def __to_string__(self, hide_password: bool = True) -> str:
        ...
        :param hide_password: Defaults to True. The password is not shown in the string
    ...
    def _get_entrypoint(self) -> Type[Dialect]:
        ...
        This is normally the dialect itself except in the case when the returned class
        implements the get_dialect_cls() method.
        """
        if "+" not in self.drivername:
    ...
    def get_dialect(self, _is_async: bool = False) -> Type[Dialect]:
        """Return the SQLAlchemy :class:`_engine.Dialect` class corresponding to this
        URL's driver name."""
