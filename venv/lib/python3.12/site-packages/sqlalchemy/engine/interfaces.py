...
class ExecuteStyle(Enum):
    """Indicates the :term:`DBAPI` cursor method that will be used to invoke a
    statement."""
    ...
    INSERTMANYVALUES = 2
    """
    Indicates cursor.execute() will be used with an INSERT where the VALUES expression
    will be expanded to accommodate for multiple parameter sets.
...
class ReflectedIdentity(TypedDict):
    ...
    :class:`_schema.Identity` construct.
    ...
    cycle: bool
    """Allows the sequence to wrap around when the maxvalue or minvalue has been
    reached."""
...
class ReflectedComputed(TypedDict):
    ...
    :class:`_schema.Computed` construct.
    ...
    sqltext: str
    """The expression used to generate this column returned as a string SQL
    expression."""
...
class ReflectedColumn(TypedDict):
    ...
    """
    Indicates that this column is computed by the database. Only some dialects return
    this key.
...
class ReflectedConstraint(TypedDict):
    """
    Dictionary representing the reflected elements corresponding to :class:`.Constraint`
...
class ReflectedIndex(TypedDict):
    """
    Dictionary representing the reflected elements corresponding to :class:`.Index`.
    ...
    """
    Expressions that compose the index.

    This list, when present, contains
    ...
    """
    Optional dict mapping column names or expressions to tuple of sort keywords, which
    may include ``asc``, ``desc``, ``nulls_first``, ``nulls_last``.
...
class ReflectedTableComment(TypedDict):
    ...
    :attr:`_schema.Table.comment` attribute.
...
class BindTyping(Enum):
    """
    Define different methods of passing typing information for bound parameters in a
    ...
    This is the default behavior for databases such as SQLite, MySQL / MariaDB, SQL
    Server.
    """
...
class Dialect(EventTarget):
    ...
    type_compiler: Any
    """
    Legacy; this is a TypeCompiler class at the class level, a TypeCompiler instance at
    the instance level.
    ...
    """This element will refer to an instance of :class:`.IdentifierPreparer` once a
    :class:`.DefaultDialect` has been constructed."""

    server_version_info: Optional[Tuple[Any, ...]]
    """
    A tuple containing a version number for the DB backend in use.

    This value is only available for supporting dialects, and is typically populated
    ...
    """Either the 'tuple' or 'list' type, depending on what cursor.execute() accepts for
    the second argument (they vary)."""
    ...
    """Deprecated; indicates if the dialect should attempt to use server side cursors by
    default."""

    supports_sane_rowcount: bool
    """Indicate whether the dialect properly implements rowcount for ``UPDATE`` and
    ``DELETE`` statements."""
    ...
    """Target database supports INSERT...VALUES with multiple value sets, i.e. INSERT
    INTO table (cols) VALUES (...), (...), (...), ..."""

    insert_executemany_returning: bool
    """Dialect / driver / database supports some means of providing INSERT...RETURNING
    support when dialect.do_executemany() is used."""

    insert_executemany_returning_sort_by_parameter_order: bool
    """Dialect / driver / database supports some means of providing INSERT...RETURNING
    support when dialect.do_executemany() is used along with the
    :paramref:`_dml.Insert.returning.sort_by_parameter_order` parameter being set."""
    ...
    """
    If True, indicates "insertmanyvalues" functionality should be used to allow for
    ``insert_executemany_returning`` behavior, if possible.
    ...
    """
    If True, and use_insertmanyvalues is also True, INSERT statements that don't include
    RETURNING will also use "insertmanyvalues".
    ...
    """
    Options indicating the database supports a form of bulk INSERT where the
    autoincrement integer primary key can be reliably used as an ordering for INSERTed
    rows.
    ...
    insertmanyvalues_page_size: int
    """
    Number of rows to render into an individual INSERT..VALUES() statement for
    :attr:`.ExecuteStyle.INSERTMANYVALUES` executions.
    ...
    """Alternate to insertmanyvalues_page_size, will additionally limit page size based
    on number of parameters total in the statement."""

    preexecute_autoincrement_sequences: bool
    """
    True if 'implicit' primary key functions must be executed separately in order to get
    their value, if RETURNING is not used.
    ...
    """
    For backends that support both a lastrowid and a RETURNING insert strategy, favor
    RETURNING for simple single-int pk inserts.
    ...
    """Target database, when given a CTE with an INSERT statement, needs the CTE to be
    below the INSERT."""

    colspecs: MutableMapping[Type[TypeEngine[Any]], Type[TypeEngine[Any]]]
    """
    A dictionary of TypeEngine classes from sqlalchemy.types mapped to subclasses that
    are specific to the dialect class.

    This dictionary is class-level only and is not accessed from the dialect instance
    itself.
    """
    ...
    """
    If True, indicates if the :paramref:`_schema.Sequence.optional` parameter on the
    :class:`_schema.Sequence` construct should signal to not generate a CREATE SEQUENCE.

    Applies only to dialects that support sequences. Currently used only to allow
    PostgreSQL SERIAL to be used on a column that specifies Sequence() for usage on
    other backends.
    """

    default_sequence_base: int
    """The default value that will be rendered as the "START WITH" portion of a CREATE
    ...
    """Indicates if Decimal objects are handled and returned for precision numeric
    types, or if floats are returned."""

    supports_native_uuid: bool
    """
    Indicates if Python UUID() objects are handled natively by the driver for SQL UUID
    datatypes.
    ...
    """
    Indicates if Python bytes() objects are returned natively by the driver for SQL
    "binary" datatypes.
    ...
    :class:`.Table` object which will be passed as "reflection options" when using
    :paramref:`.Table.autoload_with`.
    ...
    """
    A dictionary of names that will contain as values the names of pep-249 exceptions
    ("IntegrityError", "OperationalError", etc) keyed to alternate class names, to
    support the case where a DBAPI has exception classes that aren't named as they are
    referred to (e.g. IntegrityError = MyException).

    In the vast majority of cases this dictionary is empty.
    """
    ...
    """
    Indicates the dialect supports comment DDL that's inline with the definition of a
    Table or Column.

    If False, this implies that ALTER must be used to set table and column comments.
    """
    ...
    """
    Define a means of passing typing information to the database and/or driver for bound
    parameters.
    ...
    has_terminate: bool
    """Whether or not this dialect has a separate "terminate" implementation that does
    not block or require awaiting."""

    engine_config_types: Mapping[str, Any]
    """A mapping of string keys that can be in an engine config linked to type
    ...
    """
    Set of DBAPI type objects that should be included in automatic
    cursor.setinputsizes() calls.
    ...
    """
    Set of DBAPI type objects that should be excluded in automatic
    cursor.setinputsizes() calls.
    ...
    """Target database supports ORDER BY <labelname>, where <labelname> refers to a
    ...
    def get_multi_pk_constraint(
        ...
        **kw: Any,
        ...
        """
        Return information about primary key constraints in all tables in the given
        ``schema``.
    ...
    def get_temp_table_names(
        ...
        """
        Return a list of temporary table names on the given connection, if supported by
    ...
    def get_temp_view_names(
        ...
        """
        Return a list of temporary view names on the given connection, if supported by
        the underlying backend.
    ...
    def get_multi_unique_constraints(
        ...
        **kw: Any,
        ...
        """
        Return information about unique constraints in all tables in the given
        ``schema``.
    ...
    def get_multi_check_constraints(
        ...
        **kw: Any,
        ...
        """
        Return information about check constraints in all tables in the given
        ``schema``.
    ...
    def get_multi_table_options(
        ...
        **kw: Any,
        ...
        """
        Return a dictionary of options specified when the tables in the given schema
        were created.
    ...
    def get_multi_table_comment(
        ...
        **kw: Any,
        ...
        """
        Return information about the table comment in all tables in the given
        ``schema``.
    ...
    def denormalize_name(self, name: str) -> str:
        """
        Convert the given name to a case insensitive identifier for the backend if it is
    ...
    def has_table(
        ...
        **kw: Any,
        ...
        """
        For internal dialect use, check the existence of a particular table or view in
        the database.
    ...
    def _get_server_version_info(self, connection: Connection) -> Any:
        ...
        This is used by the default implementation to populate the "server_version_info"
        attribute and is called exactly once upon first connect.
        """
    ...
    def _get_default_schema_name(self, connection: Connection) -> str:
        """
        Return the string name of the currently selected schema from the given
        connection.

        This is used by the default implementation to populate the "default_schema_name"
        attribute and is called exactly once upon first connect.
        """
    ...
    def do_terminate(self, dbapi_connection: DBAPIConnection) -> None:
        """
        Provide an implementation of ``connection.close()`` that tries as much as
        possible to not block, given a DBAPI connection.
    ...
    def do_recover_twophase(self, connection: Connection) -> List[Any]:
        """
        Recover list of uncommitted prepared two phase transaction identifiers on the
        given connection.
    ...
    def _deliver_insertmanyvalues_batches(
        ...
        """Convert executemany parameters for an INSERT into an iterator of
        statement/single execute values, used by the insertmanyvalues feature."""
        raise NotImplementedError()
    ...
    def get_default_isolation_level(
        ...
        """
        Given a DBAPI connection, return its isolation level, or a default isolation
    ...
    def get_isolation_level_values(
        ...
        """
        Return a sequence of string isolation level names that are accepted by this
        dialect.
    ...
    def get_dialect_cls(cls, url: URL) -> Type[Dialect]:
        ...
        This is a hook that allows an external plugin to provide functionality around an
        existing dialect, by allowing the plugin to be loaded from the url based on an
        entrypoint, and then the plugin returns the actual dialect to be used.
...
class CreateEnginePlugin:
    """
    A set of hooks intended to augment the construction of an :class:`_engine.Engine`
    ...
    * connectivity plugins such as proxies
    ...
    def engine_created(self, engine: Engine) -> None:
        """
        Receive the :class:`_engine.Engine` object when it is fully constructed.

        The plugin may make additional changes to the engine, such as registering engine
...
class ExecutionContext:
    ...
    connection: Connection
    """
    Connection object which can be freely used by default value generators to execute
    SQL.

    This Connection should reference the same underlying connection/transactional
    ...
    statement: str
    """
    String version of the statement to be executed.

    Is either passed to the constructor, or must be created from the sql.Compiled object
    by the time pre_exec() has completed.
    """
    ...
    This should be structurally equivalent to compiled.statement, but not necessarily
    ...
    """
    A list of Column objects for which a server-side default or inline SQL expression
    value was fired off.
    ...
    def create_cursor(self) -> DBAPICursor:
        ...
        Some dialects may wish to change the behavior of connection.cursor(), such as
    ...
    def lastrow_has_defaults(self) -> bool:
        """Return True if the last INSERT or UPDATE row contained inlined or database-
        side defaults."""
    ...
    def get_rowcount(self) -> Optional[int]:
        """
        Return the DBAPI ``cursor.rowcount`` value, or in some cases an interpreted
        value.
...
class ExceptionContext:
    ...
    This member is present in all cases except for when handling an error within the
    connection pool "pre-ping" process.
    """
    ...
    """
    The :class:`sqlalchemy.exc.StatementError` which wraps the original, and will be
    raised if exception handling is not circumvented by the event.

    May be None, as not all exception types are wrapped by SQLAlchemy. For DBAPI-level
    ...
    """
    The exception that was returned by the previous handler in the exception chain, if
    any.

    If present, this exception will be the one ultimately raised by SQLAlchemy unless a
    ...
    """
    The :class:`.ExecutionContext` corresponding to the execution operation in progress.
    ...
    """
    Represent whether all connections in the pool should be invalidated when a
    ...
    is_pre_ping: bool
    """
    Indicates if this error is occurring within the "pre-ping" step performed when
    :paramref:`_sa.create_engine.pool_pre_ping` is set to ``True``.  In this mode, the
    :attr:`.ExceptionContext.engine` attribute will be ``None``.  The dialect in use is
    accessible via the :attr:`.ExceptionContext.dialect` attribute.
...
class AdaptedConnection:
    ...
    def run_async(self, fn: Callable[[Any], Awaitable[_T]]) -> _T:
        """
        Run the awaitable returned by the given function, which is passed the raw
        asyncio driver connection.
