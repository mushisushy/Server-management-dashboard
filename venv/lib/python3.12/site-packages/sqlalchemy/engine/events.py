...
class ConnectionEvents(event.Events[ConnectionEventsTarget]):
    """
    Available events for :class:`_engine.Connection` and :class:`_engine.Engine`.
    ...
    def before_execute(
        ...
        """
        Intercept high level execute() events, receiving uncompiled SQL constructs and
    ...
    def before_cursor_execute(
        ...
        """
        Intercept low-level cursor execute() events before execution, receiving the
    ...
    def set_connection_execution_options(
        ...
        """
        Intercept when the :meth:`_engine.Connection.execution_options` method is
        called.
...
class DialectEvents(event.Events[Dialect]):
    ...
    def handle_error(
        ...
        r"""
        Intercept all exceptions processed by the :class:`_engine.Dialect`, typically
        but not limited to those emitted within the scope of a
        :class:`_engine.Connection`.
    ...
    def do_executemany(
        ...
        Return the value True to halt further events from invoking, and to indicate that
        the cursor execution has already taken place within the event handler.
        """
