...
class MySQLCompiler(compiler.SQLCompiler):
    ...
    def default_from(self):
        """Called when a ``SELECT`` statement has no froms, and no ``FROM`` clause is to
        be appended."""
        if self.stack:
    ...
    def visit_match_op_binary(self, binary, operator, **kw):
        """Note that `mysql_boolean_mode` is enabled by default because of backward
        compatibility."""
...
class MySQLTypeCompiler(compiler.GenericTypeCompiler):
    ...
    def _extend_string(self, type_, defaults, spec):
        """Extend a string-type declaration with standard SQL CHARACTER SET / COLLATE
        annotations and MySQL specific extensions."""
...
class MySQLDialect(default.DefaultDialect):
    ...
    def _tabledef_parser(self):
        ...
        The deferred creation ensures that the dialect has retrieved server version
        information first.
        """
...
class _DecodingRow:
    ...
    Smooth over data type issues (esp. with alpha driver versions) and normalize strings
    as Unicode regardless of user-configured driver encoding settings.
    """
