...
"""
The inspection module provides the :func:`_sa.inspect` function, which delivers runtime
information about a wide variety of SQLAlchemy objects, both within the Core as well as
the ORM.
...
class Inspectable(Generic[_T]):
    ...
    This allows typing to set up a linkage between an object that can be inspected and
    the type of inspection it returns.

    Unfortunately we cannot at the moment get all classes that are returned by
    inspection to suit this interface as we get into MRO issues.
    """
...
class _InspectableTypeProtocol(Protocol[_TCov]):
    """A protocol defining a method that's used when a type (ie the class itself) is
    passed to inspect()."""
...
class _InspectableProtocol(Protocol[_TCov]):
    """A protocol defining a method that's used when an instance is passed to
    inspect()."""
