...
def fixup_container_fwd_refs(
    ...
    """Correct dict['x', 'y'] into dict[ForwardRef('x'), ForwardRef('y')] and similar
    for list, set."""
...
def pep695_values(type_: _AnnotationScanType) -> Set[Any]:
    """
    Extracts the value from a TypeAliasType, recursively exploring unions and inner
    TypeAliasType to flatten them into a single set.

    Forward references are not evaluated, so no recursive exploration happens into them.
    """
    _seen = set()
...
def de_optionalize_union_types(
    ...
    """
    Given a type, filter out ``Union`` types that include ``NoneType`` to not include
    the ``NoneType``.
...
def _de_optionalize_fwd_ref_union_types(
    ...
    """
    Return the non-optional type for Optional[], Union[None, ...], x|None, etc. without
    de-stringifying forward refs.
...
def is_origin_of_cls(
    ...
    """Return True if the given type has an __origin__ that shares a base with the given
    class."""
...
def is_origin_of(type_: Any, *names: str, module: Optional[str] = None) -> bool:
    """Return True if the given type has an __origin__ with the given name and optional
    module."""
...
class CallableReference(Generic[_FN]):
    ...
    works around mypy's limitation of not allowing callables assigned as instance
    variables
    """
