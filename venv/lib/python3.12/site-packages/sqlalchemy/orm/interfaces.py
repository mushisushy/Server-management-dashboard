...
This module and the classes within are mostly private, though some attributes are
exposed when inspecting mappings.
"""
...
class _IntrospectsAnnotations:
    ...
    def found_in_pep593_annotated(self) -> Any:
        """Return a copy of this object to use in declarative when the object is found
        inside of an Annotated object."""
...
class _AttributeOptions(NamedTuple):
    ...
    def _get_arguments_for_make_dataclass(
        ...
        """Given attribute key, annotation, and value from a class, return the argument
        tuple we would pass to dataclasses.make_dataclass() for this attribute."""
...
class _DCAttributeOptions:
    """
    Mixin for descriptors or configurational objects that include dataclass field
    options.
...
class _MapsColumns(_DCAttributeOptions, _MappedAttribute[_T]):
    """Interface for declarative-capable construct that delivers one or more Column
    objects to the declarative process to be part of a Table."""
    ...
    def columns_to_assign(self) -> List[Tuple[Column[_T], int]]:
        """A list of Column objects that should be declaratively added to the new Table
        object."""
        raise NotImplementedError()
...
class MapperProperty(
    ...
    """The :class:`_orm.PropComparator` instance that implements SQL expression
    construction on behalf of this mapped attribute."""
    ...
    info: _InfoType
    """
    Info dictionary associated with the object, allowing user-defined data to be
    associated with this :class:`.InspectionAttr`.
    ...
    def setup(
        ...
        **kwargs: Any,
        ...
        Each MapperProperty associated with the target mapper processes the statement
    ...
    def create_row_processor(
        ...
        """Produce row processing functions and append to the given set of populators
        lists."""
    ...
    def cascade_iterator(
        ...
        Note that the 'cascade' collection on this MapperProperty is checked first for
    ...
    def set_parent(self, parent: Mapper[Any], init: bool) -> None:
        ...
        This method is overridden by some subclasses to perform extra setup when the
    ...
    def instrument_class(self, mapper: Mapper[Any]) -> None:
        """
        Hook called by the Mapper to the property to initiate instrumentation of the
        class attribute managed by this MapperProperty.

        The MapperProperty here will typically call out to the attributes module to set
        up an InstrumentedAttribute.

        This step is the first of two steps to set up an InstrumentedAttribute, and is
        called early in the mapper setup process.

        The second step is typically the init_class_attribute step, called from
        StrategizedProperty via the post_instrument_class() hook.  This step assigns
        additional state to the InstrumentedAttribute (specifically the "impl") which
        has been determined after the MapperProperty has determined what kind of
        persistence management it needs to do (e.g. scalar, object, collection, etc).
        """
    ...
    def post_instrument_class(self, mapper: Mapper[Any]) -> None:
        ...
        The given Mapper is the Mapper invoking the operation, which may not be the same
        Mapper as self.parent in an inheritance scenario; however, Mapper will always at
        least be a sub-mapper of self.parent.

        This method is typically used by StrategizedProperty, which delegates it to
...
class PropComparator(SQLORMOperations[_T_co], Generic[_T_co], ColumnOperators):
    ...
    def _bulk_update_tuples(
        ...
        """
        Receive a SQL expression that represents a value in the SET clause of an UPDATE
        statement.
    ...
    def _parentmapper(self) -> Mapper[Any]:
        """Legacy; this is renamed to _parententity to be compatible with
        QueryableAttribute."""
    ...
    def adapter(self) -> Optional[_ORMAdapterProto]:
        """Produce a callable that adapts column expressions to suit an aliased version
        of this comparator."""
    ...
    def of_type(self, class_: _EntityType[Any]) -> PropComparator[_T_co]:
        ...
        :func:`_orm.with_polymorphic` construct, or :func:`_orm.aliased` construct.
    ...
    def and_(self, *criteria: _ColumnExpressionArgument[bool]) -> PropComparator[bool]:
        """
        Add additional criteria to the ON clause that's represented by this relationship
        attribute.
    ...
    def any(
        ...
        **kwargs: Any,
        ...
        r"""
        Return a SQL expression representing true if this element references a member
...
class ORMOption(ExecutableOption):
    ...
    """If True, indicate this option should be carried along to "secondary" SELECT
    ...
    def _adapt_cached_option_to_uncached_option(
        ...
        """
        Adapt this option to the "uncached" version of itself in a loader strategy
        context.
...
class CompileStateOption(HasCacheKey, ORMOption):
    """
    Base for :class:`.ORMOption` classes that affect the compilation of a SQL query and
    therefore need to be part of the cache key.
    ...
    def process_compile_state_replaced_entities(
        ...
        """
        Apply a modification to a given :class:`.ORMCompileState`, given entities that
        were replaced by with_only_columns() or with_entities().
...
class CriteriaOption(CompileStateOption):
    ...
    def get_global_criteria(self, attributes: Dict[str, Any]) -> None:
        """Update additional entity criteria options in the given attributes
        dictionary."""
...
class UserDefinedOption(ORMOption):
    ...
    propagate_to_loaders = False
    """If True, indicate this option should be carried along to "secondary" Query
    objects produced during lazy loads or refresh operations."""
...
class MapperOption(ORMOption):
    ...
    def process_query_conditionally(self, query: Query[Any]) -> None:
        """
        Same as process_query(), except that this option may not apply to the given
        query.

        This is typically applied during a lazy load or scalar refresh operation to
        propagate options stated in the original Query to the new Query being used for
        the load.  It occurs for those options that specify propagate_to_loaders=True.
        """
...
class LoaderStrategy:
    ...
    * It produces "row processor" functions at result fetching time.
    ...
    def create_row_processor(
        ...
        StrategizedProperty delegates its create_row_processor() method directly to this
        method.
        """
