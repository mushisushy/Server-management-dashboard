...
class LoaderCallableStatus(Enum):
    PASSIVE_NO_RESULT = 0
    """Symbol returned by a loader callable or other attribute/history retrieval
    operation when a value could not be determined, based on loader callable flags."""

    PASSIVE_CLASS_MISMATCH = 1
    """
    Symbol indicating that an object is locally present for a given primary key identity
    but it is not of the requested class.

    The return value is therefore None and no SQL should be emitted.
    """

    ATTR_WAS_SET = 2
    """Symbol returned by a loader callable to indicate the retrieved value, or values,
    ...
    NO_VALUE = 4
    """Symbol which may be placed as the 'previous' value of an attribute, indicating no
    value was loaded for an attribute when it was modified, and flags indicated we were
    not to load it."""
...
class PassiveFlag(FastIntFlag):
    ...
    NO_CHANGE = 0
    """No callables or SQL should be emitted on attribute access and no state should
    change."""
    ...
    RELATED_OBJECT_OK = 4
    """Callables can use SQL to load related objects as well as scalar value
    attributes."""

    INIT_OK = 8
    """Attributes should be initialized with a blank value (None or an empty collection)
    upon get, if no other value can be obtained."""
...
class EventConstants(Enum):
    ...
    NO_KEY = 4
    """
    Indicates an :class:`.AttributeEvent` event that did not have any key argument.
...
class RelationshipDirection(Enum):
    """
    Enumeration which indicates the 'direction' of a :class:`_orm.RelationshipProperty`.
    ...
    This symbol is typically used by the internals but may be exposed within certain API
    features.
    """
...
class InspectionAttrExtensionType(Enum):
    """Symbols indicating the type of extension that a :class:`.InspectionAttr` is part
    of."""
...
class NotExtension(InspectionAttrExtensionType):
    ...
    """
    Symbol indicating an :class:`InspectionAttr` that's not part of sqlalchemy.ext.
...
def _mapper_or_none(
    ...
    """Return the :class:`_orm.Mapper` for the given class or None if the class is not
    mapped."""
...
def _is_mapped_class(entity: Any) -> bool:
    """Return True if the given object is a mapped class, :class:`_orm.Mapper`, or
    :class:`.AliasedClass`."""
...
def class_mapper(class_: Type[_O], configure: bool = True) -> Mapper[_O]:
    """
    Given a class, return the primary :class:`_orm.Mapper` associated with the key.
...
class InspectionAttr:
    """
    A base class applied to all ORM objects and attributes that are related to things
    that can be returned by the :func:`_sa.inspect` function.

    The attributes defined here allow the usage of simple boolean checks to test basic
    facts about the object returned.

    While the boolean checks here are basically the same as using the Python
    isinstance() function, the flags here can be used without the need to import all of
    these classes, and also such that the SQLAlchemy class system can change while
    leaving the flags here intact for forwards-compatibility.
    """
    ...
    :attr:`.interfaces.NotExtension.NOT_EXTENSION`
...
class ORMDescriptor(Generic[_T_co], TypingOnly):
    """Represent any Python descriptor that provides a SQL expression construct at the
    class level."""
...
class _MappedAnnotationBase(Generic[_T_co], TypingOnly):
    ...
    these are classes that can appear on the left side of an ORM declarative mapping,
    containing a mapped class or in some cases a collection surrounding a mapped class.
    """
...
class SQLORMExpression(SQLORMOperations[_T_co], SQLColumnExpression[_T_co], TypingOnly):
    """
    A type that may be used to indicate any ORM-level attribute or object that acts in
    place of one, in the context of SQL expression construction.
...
class _DeclarativeMapped(Mapped[_T_co], _MappedAttribute[_T_co]):
    """Mixin for :class:`.MapperProperty` subclasses that allows them to be compatible
    with ORM-annotated declarative mappings."""
