...
class _InstallLoaderCallableProto(Protocol[_O]):
    """
    Used at result loading time to install a _LoaderCallable callable upon a specific
    InstanceState, which will be used to populate an attribute when that attribute is
    accessed.

    Concrete examples are per-instance deferred column loaders and relationship lazy
    loaders.
    """
...
class InstanceState(interfaces.InspectionAttrInfo, Generic[_O]):
    ...
    """A weak reference, or in the default case a plain callable, that returns a
    reference to the current :class:`.IdentityMap`, if any."""
    if not TYPE_CHECKING:
    ...
    """
    The set of keys which are 'expired' to be loaded by the manager's deferred scalar
    ...
    In SQLAlchemy 1.0, this is only used for lazy loaders / deferred loaders that were
    set up via query option.

    Previously, callables was used also to indicate expired attributes by storing a link
    to the InstanceState itself in this dictionary. This role is now handled by the
    ...
    def attrs(self) -> util.ReadOnlyProperties[AttributeState]:
        """
        Return a namespace representing each attribute on the mapped object, including
    ...
    def was_deleted(self) -> bool:
        """
        Return True if this object is or was previously in the "deleted" state and has
    ...
    def session(self) -> Optional[Session]:
        """
        Return the owning :class:`.Session` for this instance, or ``None`` if none
        available.
    ...
    def async_session(self) -> Optional[AsyncSession]:
        """
        Return the owning :class:`_asyncio.AsyncSession` for this instance, or ``None``
        if none available.
    ...
    def _cleanup(self, ref: weakref.ref[_O]) -> None:
        ...
        this _cleanup **assumes** that there are no strong refs to us! Will not work
        otherwise!
        """
    ...
    def _load_expired(
        ...
        """__call__ allows the InstanceState to act as a deferred callable for loading
        expired attributes, which is also serializable (picklable)."""
    ...
    def unloaded_expirable(self) -> Set[str]:
        ...
        This attribute was added as an implementation-specific detail at some point and
    ...
    def _commit(self, dict_: _InstanceDict, keys: Iterable[str]) -> None:
        ...
        This is used by a partial-attribute load operation to mark committed those
        attributes which were refreshed from the database.
...
class AttributeState:
    """
    Provide an inspection interface corresponding to a particular attribute on a
    ...
    def loaded_value(self) -> Any:
        ...
        If the value has not been loaded, or is otherwise not present in the object's
...
class PendingCollection:
    ...
    Stores items appended to and removed from a collection that has not yet been loaded.
    When the collection is loaded, the changes stored in PendingCollection are applied
    to it to produce the final result.
    """
