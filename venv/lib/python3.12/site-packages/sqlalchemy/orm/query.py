...
class Query(
    ...
    def statement(self) -> Union[Select[_T], FromStatement[_T], UpdateBase]:
        ...
        The statement by default will not have disambiguating labels applied to the
    ...
    def _final_statement(self, legacy_query_style: bool = True) -> Select[Any]:
        ...
        This is the Core-only select() that will be rendered by a complete compilation
        of this query, and is what .statement used to return in 1.3.
        """
    ...
    def subquery(
        ...
        """
        Return the full SELECT statement represented by this :class:`_query.Query`,
        embedded within an :class:`_expression.Alias`.
    ...
    def enable_assertions(self, value: bool) -> Self:
        ...
        When set to False, the returned Query will not assert its state before certain
        operations, including that LIMIT/OFFSET has not been applied when filter() is
        called, no criterion exists when get() is called, and no "from_statement()"
        exists when filter()/order_by()/group_by() etc. is called.  This more permissive
        mode is used by custom Query subclasses to specify criterion or other modifiers
        outside of the usual usage patterns.

        Care should be taken to ensure that the usage pattern is even possible.  A
        statement applied by from_statement() will override any criterion set by
    ...
    def _with_current_path(self, path: PathRegistry) -> Self:
        ...
        Used by deferred loaders (see strategies.py) which transfer query options from
        an originating query to a newly generated query intended for the deferred load.
        """
    ...
    def get(self, ident: _PKIdentityArgument) -> Optional[Any]:
        """
        Return an instance based on the given primary key identifier, or ``None`` if not
        found.
    ...
    def lazy_loaded_from(self) -> Optional[InstanceState[Any]]:
        """
        An :class:`.InstanceState` that is using this :class:`_query.Query` for a lazy
        load operation.
    ...
    def correlate(
        ...
        *fromclauses: Union[Literal[None, False], _FromClauseArgument],
        ...
        """
        Return a :class:`.Query` construct which will correlate the given FROM clauses
        to that of an enclosing :class:`.Query` or :func:`~.expression.select`.
    ...
    def populate_existing(self) -> Self:
        """
        Return a :class:`_query.Query` that will expire and refresh all instances as
    ...
    def _with_invoke_all_eagers(self, value: bool) -> Self:
        """
        Set the 'invoke all eagers' flag which causes joined- and subquery loaders to
        traverse into already-loaded related objects and collections.
    ...
    def with_parent(
        ...
        collection, using its attribute state as well as an established
        :func:`_orm.relationship()` configuration.
    ...
    def values(self, *columns: _ColumnsClauseArgument[Any]) -> Iterable[Any]:
        """Return an iterator yielding result tuples corresponding to the given list of
        columns."""
    ...
    def with_entities(
        ...
        r"""
        Return a new :class:`_query.Query` replacing the SELECT list with the given
        entities.
    ...
    def options(self, *args: ExecutableOption) -> Self:
        """
        Return a new :class:`_query.Query` object, applying the given list of mapper
        options.
    ...
    def with_for_update(
        ...
        *,
        ...
        """
        Return a new :class:`_query.Query` with the specified options for the ``FOR
        UPDATE`` clause.
    ...
    def params(self, __params: Optional[Dict[str, Any]] = None, **kw: Any) -> Self:
        ...
        convenient, however some parameter dictionaries contain unicode keys in which
        case \**kwargs cannot be used.
        """
        if __params:
            kw.update(__params)
    ...
    def filter(self, *criterion: _ColumnExpressionArgument[bool]) -> Self:
        r"""
        Apply the given filtering criterion to a copy of this :class:`_query.Query`,
    ...
    def _filter_by_zero(self) -> Any:
        """For the filter_by() method, return the target entity for which we will
        attempt to derive an expression from based on string name."""
    ...
    def order_by(
        ...
        *clauses: _ColumnExpressionOrStrLabelArgument[Any],
        ...
        """
        Apply one or more ORDER BY criteria to the query and return the newly resulting
        :class:`_query.Query`.
    ...
    def join(
        ...
        *,
        ...
        r"""
        Create a SQL JOIN against this :class:`_query.Query` object's criterion and
        apply generatively, returning the newly resulting :class:`_query.Query`.
    ...
    def outerjoin(
        ...
        *,
        ...
        """
        Create a left outer join against this ``Query`` object's criterion and apply
    ...
    def reset_joinpoint(self) -> Self:
        """
        Return a new :class:`.Query`, where the "join point" has been reset back to the
        base FROM entities of the query.
    ...
    def slice(
        ...
        """
        Computes the "slice" of the :class:`_query.Query` represented by the given
        indices and returns the resulting :class:`_query.Query`.
    ...
    def first(self) -> Optional[_T]:
        """
        Return the first result of this ``Query`` or None if the result doesn't contain
        any row.
    ...
    def scalar(self) -> Any:
        """
        Return the first element of the first result or None if no rows present.  If
        multiple rows are returned, raises :class:`_exc.MultipleResultsFound`.
    ...
    def exists(self) -> Exists:
        """
        A convenience method that turns a query into an EXISTS subquery of the form
    ...
    def _compile_state(self, for_statement: bool = False, **kw: Any) -> ORMCompileState:
        ...
        The ORMCompileState object is normally created directly as a result of the
        SQLCompiler.process() method being handed a Select() or FromStatement() object
        that uses the "orm" plugin.   This method provides a means of creating this
        ORMCompileState object directly without using the compiler.

        This method is used only for deprecated cases, which include the .from_self()
        method for a Query that has multiple levels of .from_self() in use, as well as
        the instances() method.  It is also used within the test suite to generate
        ORMCompileState objects for test purposes.
        """
