...
class _InstrumentationEventsHold:
    """Temporary marker object used to transfer from _accept_with() to _listen() on the
    InstrumentationEvents class."""
...
class InstanceEvents(event.Events[ClassManager[Any]]):
    ...
    * the :class:`_orm.Mapper` class itself indicates listening for all
    ...
    def _sa_event_merge_wo_load(self, target: _O, context: QueryContext) -> None:
        """
        Receive an object instance after it was the subject of a merge() call, when
        load=False was passed.
    ...
    def load(self, target: _O, context: QueryContext) -> None:
        """
        Receive an object instance after it has been created via ``__new__``, and after
    ...
    def refresh(
        ...
        """
        Receive an object instance after one or more attributes have been refreshed from
        a query.
    ...
    def refresh_flush(
        ...
        """
        Receive an object instance after one or more attributes that contain a column-
        level default or onupdate handler have been refreshed during persistence of the
        object's state.
...
class _EventsHold(event.RefCollection[_ET]):
    ...
    Establish _listen() for that class' mapper/instrumentation when those objects are
...
class MapperEvents(event.Events[mapperlib.Mapper[Any]]):
    ...
    * the :class:`_orm.Mapper` class itself indicates listening for all
    ...
    def instrument_class(self, mapper: Mapper[_O], class_: Type[_O]) -> None:
        r"""
        Receive a class when the mapper is first constructed, before instrumentation is
        applied to the mapped class.
    ...
    def after_mapper_constructed(self, mapper: Mapper[_O], class_: Type[_O]) -> None:
        """
        Receive a class and mapper when the :class:`_orm.Mapper` has been fully
        constructed.
    ...
    def mapper_configured(self, mapper: Mapper[_O], class_: Type[_O]) -> None:
        r"""
        Called when a specific mapper has completed its own configuration within the
        scope of the :func:`.configure_mappers` call.
    ...
    def before_insert(
        ...
        """
        Receive an object instance before an INSERT statement is emitted corresponding
        to that instance.
    ...
    def after_insert(
        ...
        """
        Receive an object instance after an INSERT statement is emitted corresponding to
        that instance.
    ...
    def before_update(
        ...
        """
        Receive an object instance before an UPDATE statement is emitted corresponding
        to that instance.
    ...
    def after_update(
        ...
        """
        Receive an object instance after an UPDATE statement is emitted corresponding to
        that instance.
    ...
    def before_delete(
        ...
        """
        Receive an object instance before a DELETE statement is emitted corresponding to
        that instance.
...
class SessionEvents(event.Events[Session]):
    ...
    def do_orm_execute(self, orm_execute_state: ORMExecuteState) -> None:
        """
        Intercept statement executions that occur on behalf of an ORM :class:`.Session`
        object.
    ...
    def after_bulk_update(self, update_context: _O) -> None:
        """
        Event for after the legacy :meth:`_orm.Query.update` method has been called.
    ...
    def after_bulk_delete(self, delete_context: _O) -> None:
        """
        Event for after the legacy :meth:`_orm.Query.delete` method has been called.
...
class QueryEvents(event.Events[Query[Any]]):
    """
    Represent events within the construction of a :class:`_query.Query` object.
    ...
    def before_compile(self, query: Query[Any]) -> None:
        """
        Receive the :class:`_query.Query` object before it is composed into a core
