...
class AbstractORMCompileState(CompileState):
    ...
    def create_for_statement(
        ...
        **kw: Any,
        ...
        For a Select object, this would be invoked from SQLCompiler.visit_select(). For
        the special FromStatement object used by Query to indicate
        "Query.from_statement()", this is called by FromStatement._compiler_dispatch()
        that would be called by SQLCompiler.process().
        """
...
class ORMCompileState(AbstractORMCompileState):
    ...
    def _lead_mapper_entities(self):
        ...
        Does **not** include entities that have been replaced by with_entities(),
        with_only_columns()
        """
    ...
    def _create_with_polymorphic_adapter(self, ext_info, selectable):
        """
        Given MapperEntity or ORMColumnEntity, setup polymorphic loading if called for
        by the Mapper.

        As of #8168 in 2.0.0rc1, polymorphic adapters, which greatly increase the
        complexity of the query creation process, are not used at all except in the
        quasi-legacy cases of with_polymorphic referring to an alias and/or subquery.
        This would apply to concrete polymorphic loading, and joined inheritance where a
        subquery is passed to with_polymorphic (which is completely unnecessary in
        modern use).
        """
        if (
...
class _DMLBulkInsertReturningColFilter(_DMLReturningColFilter):
    """
    An adapter used for the DML RETURNING case specifically for ORM bulk insert (or any
    hypothetical DML that is splitting out a class hierarchy among multiple DML
    statements....ORM bulk insert is the only example right now)

    its main job is to limit the columns in a RETURNING to only a specific mapped table
    in a hierarchy.
    """
...
class _DMLUpdateDeleteReturningColFilter(_DMLReturningColFilter):
    """
    An adapter used for the DML RETURNING case specifically for ORM enabled
    UPDATE/DELETE.
...
class ORMFromStatementCompileState(ORMCompileState):
    ...
    def setup_dml_returning_compile_state(self, dml_mapper):
        """Used by BulkORMInsert, Update, Delete to set up a handler for RETURNING to
        return ORM objects and expressions."""
...
class ORMSelectCompileState(ORMCompileState, SelectState):
    ...
    def _create_entities_collection(cls, query, legacy):
        """
        Creates a partial ORMSelectCompileState that includes the full collection of
        _MapperEntity and other _QueryEntity objects.

        Supports a few remaining use cases that are pre-compilation but still need to
        gather some of the column  / adaption information.
        """
        self = cls.__new__(cls)
    ...
    def _entity_zero(self):
        """Return the 'entity' (mapper or AliasedClass) associated with the first
        QueryEntity, or alternatively the 'select from' entity if specified."""
    ...
    def _join_left_to_right(
        ...
        """Given raw "left", "right", "onclause" parameters consumed from a particular
        key within _join(), add a real ORMJoin object to our _from_obj list (or augment
        an existing one)"""
    ...
    def _join_place_explicit_left_side(self, entities_collection, left):
        """When join conditions express a left side explicitly, determine where in our
        existing list of FROM clauses we should join towards, or if we need to make a
        new join, and if so is it from one of our existing entities."""
    ...
    def _join_check_and_adapt_right_side(self, left, right, onclause, prop):
        """Transform the "right" side of the join as well as the onclause according to
        polymorphic mapping translations, aliasing on the query or on the join, special
        cases where the right and left side have overlapping tables."""
    ...
    def _adjust_for_extra_criteria(self):
        ...
        clause of this query, as well as the "select from entity", add criterion to the
        WHERE clause of the given QueryContext such that only the appropriate subtypes
        are selected from the total results.
