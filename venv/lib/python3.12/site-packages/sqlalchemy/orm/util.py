...
class ORMAdapter(sql_util.ColumnAdapter):
    """ColumnAdapter subclass which excludes adaptation of entities from non-matching
    mappers."""
...
class AliasedInsp(
    ...
    """
    Provide an inspection interface for an :class:`.AliasedClass` object.
    ...
    * ``polymorphic_on`` - an alternate column or SQL expression which
    ...
    In the vast majority of cases, this is the mapped class.  However it may also be
    another AliasedClass (alias of alias).
    """
...
class _WrapUserEntity:
    """
    A wrapper used within the loader_criteria lambda caller so that we can bypass
    declared_attr descriptors on unmapped mixins, which normally emit a warning for such
    use.
...
class LoaderCriteriaOption(CriteriaOption):
    """
    Add additional WHERE criteria to the load for all occurrences of a particular
    entity.
...
class Bundle(
    ...
    """
    A grouping of SQL expressions that are returned by a :class:`.Query` under one
    namespace.
    ...
    single_entity = False
    """If True, queries for a single Bundle will be returned as a single entity, rather
...
def _orm_annotate(element: _SA, exclude: Optional[Any] = None) -> _SA:
    """
    Deep copy the given ClauseElement, annotating each element with the "_orm_adapt"
    flag.
...
def with_parent(
    ...
    related instance, using established :func:`_orm.relationship()` configuration.
...
def _entity_corresponds_to(
    ...
    """Determine if 'given' corresponds to 'entity', in terms of an entity passed to
    Query that would match the same entity being referred to elsewhere in the query."""
    if insp_is_aliased_class(entity):
...
def _entity_corresponds_to_use_path_impl(
    ...
    """
    Determine if 'given' corresponds to 'entity', in terms of a path of loader options
    where a mapped attribute is taken to be a member of a parent entity.
...
def _entity_isa(given: _InternalEntityType[Any], mapper: Mapper[Any]) -> bool:
    """Determine if 'given' "is a" mapper, in terms of the given would load rows of type
    'mapper'."""
...
def _getitem(iterable_query: Query[Any], item: Any) -> Any:
    """Calculate __getitem__ in terms of an iterable query object that also has a
    slice() method."""
...
def _extract_mapped_subtype(
    ...
    """
    Given an annotation, figure out if it's ``Mapped[something]`` and if so, return the
    ``something`` part.
