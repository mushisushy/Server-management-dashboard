...
class AbstractCollectionWriter(Generic[_T]):
    """Virtual collection which includes append/remove methods that synchronize into the
...
class WriteOnlyCollection(AbstractCollectionWriter[_T]):
    ...
    def select(self) -> Select[Tuple[_T]]:
        """Produce a :class:`_sql.Select` construct that represents the rows within this
        instance-local :class:`_orm.WriteOnlyCollection`."""
    ...
    def insert(self) -> Insert:
        """
        For one-to-many collections, produce a :class:`_dml.Insert` which will insert
        new rows in terms of this this instance-local :class:`_orm.WriteOnlyCollection`.
    ...
    def update(self) -> Update:
        """Produce a :class:`_dml.Update` which will refer to rows in terms of this
        instance-local :class:`_orm.WriteOnlyCollection`."""
        return update(self.attr.target_mapper).where(*self._where_criteria)

    def delete(self) -> Delete:
        """Produce a :class:`_dml.Delete` which will refer to rows in terms of this
        instance-local :class:`_orm.WriteOnlyCollection`."""
    ...
    def add_all(self, iterator: Iterable[_T]) -> None:
        ...
        The given items will be persisted to the database in terms of the parent
        instance's collection on the next flush.
        """
    ...
    def add(self, item: _T) -> None:
        ...
        The given item will be persisted to the database in terms of the parent
        instance's collection on the next flush.
        """
    ...
    def remove(self, item: _T) -> None:
        ...
        The given item will be removed from the parent instance's collection on the next
        flush.
        """
