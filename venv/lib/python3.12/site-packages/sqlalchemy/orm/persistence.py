...
# mypy: ignore-errors
"""
Private module containing functions used to emit INSERT, UPDATE and DELETE statements on
behalf of a :class:`_orm.Mapper` and its descending mappers.
...
def save_obj(base_mapper, states, uowtransaction, single=False):
    ...
    given a list of states to be flushed.  The base mapper in an inheritance hierarchy
    handles the inserts/ updates for all descendant mappers.
    """
...
def post_update(base_mapper, states, uowtransaction, post_update_cols):
    """Issue UPDATE statements on behalf of a relationship() which specifies
    post_update."""
...
def _organize_states_for_save(base_mapper, states, uowtransaction):
    ...
    This includes splitting out into distinct lists for each, calling
    before_insert/before_update, obtaining key information for each state including its
    dictionary, mapper, the connection to use for the execution per state, and the
    identity flag.
    """
...
def _organize_states_for_post_update(base_mapper, states, uowtransaction):
    ...
    This includes obtaining key information for each state including its dictionary,
...
def _organize_states_for_delete(base_mapper, states, uowtransaction):
    ...
    This includes calling out before_delete and obtaining key information for each state
    including its dictionary, mapper, the connection to use for the execution per state.
    """
    for state, dict_, mapper, connection in _connections_for_states(
        base_mapper, uowtransaction, states
    ):
...
def _collect_update_commands(
    ...
    *,
    ...
    This function works intricately with the history system to determine exactly what
    values should be updated as well as how the row should be matched within an UPDATE
    statement.  Includes some tricky scenarios where the primary key of an object might
    have been changed.
    """
...
def _collect_post_update_commands(
    ...
    """Identify sets of values to use in UPDATE statements for a list of states within a
    post_update operation."""
...
def _collect_delete_commands(base_mapper, uowtransaction, table, states_to_delete):
    """Identify values to use in DELETE statements for a list of states to be
    deleted."""
...
def _emit_update_statements(
    ...
    *,
    ...
    """Emit UPDATE statements corresponding to value lists collected by
...
def _emit_insert_statements(
    ...
    *,
    ...
    """Emit INSERT statements corresponding to value lists collected by
...
def _emit_delete_statements(base_mapper, uowtransaction, mapper, table, delete):
    """Emit DELETE statements corresponding to value lists collected by
...
def _finalize_insert_update_commands(base_mapper, uowtransaction, states):
    """Finalize state on states that have been inserted or updated, including calling
    after_insert/after_update events."""
...
def _postfetch(
    ...
    """Expire attributes in need of newly persisted database state, after an INSERT or
    UPDATE statement has proceeded for that state."""
...
def _connections_for_states(base_mapper, uowtransaction, states):
    ...
    The states are sorted according to _sort_states, then paired with the connection
    they should be using for the given unit of work transaction.
    """
