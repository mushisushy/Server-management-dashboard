...
class Mapper(
    ...
    """
    Defines an association between a Python class and a database table or other
    ...
    """
    A sequence containing the collection of :class:`_schema.Table` or
    :class:`_schema.TableClause` objects which this :class:`_orm.Mapper` is aware of.
    ...
    :func:`_orm.validates` decorator.

    The dictionary contains string attribute names as keys mapped to the actual
    validation method.
    """
    ...
    """
    The immediate :class:`_expression.FromClause` to which this :class:`_orm.Mapper`
    refers.
    ...
    """References the :class:`_orm.Mapper` which this :class:`_orm.Mapper` inherits
    from, if any."""
    ...
    This is a *read only* attribute determined during mapper construction. Behavior is
    undefined if directly modified.
    """

    primary_key: Tuple[Column[Any], ...]
    """
    An iterable containing the collection of :class:`_schema.Column` objects which
    comprise the 'primary key' of the mapped table, from the perspective of this
    :class:`_orm.Mapper`.
    ...
    """
    The :class:`.ClassManager` which maintains event listeners and class-bound
    descriptors for this :class:`_orm.Mapper`.
    ...
    single: bool
    """
    Represent ``True`` if this :class:`_orm.Mapper` is a single table inheritance
    mapper.
    ...
    non_primary: bool
    """
    Represent ``True`` if this :class:`_orm.Mapper` is a "non-primary" mapper, e.g. a
    mapper that is used only to select rows but not for persistence management.
    ...
    """
    The :class:`_schema.Column` or SQL expression specified as the ``polymorphic_on``
    argument for this :class:`_orm.Mapper`, within an inheritance scenario.
    ...
    """
    A mapping of "polymorphic identity" identifiers mapped to :class:`_orm.Mapper`
    ...
    """
    A collection of :class:`_schema.Column` or other scalar expression objects
    ...
    def _configure_inheritance(self):
        """Configure settings related to inheriting and/or inherited mappers being
        present."""
    ...
    def _set_concrete_base(self, mapper):
        ...
        :class:`_orm.Mapper`, assuming this :class:`_orm.Mapper` is concrete and does
        not already have an inherits."""
    ...
    def _configure_class_instrumentation(self):
        ...
        associate this Mapper with the given class and entity name.
    ...
    def _configure_polymorphic_setter(self, init=False):
        """
        Configure an attribute on the mapper representing the 'polymorphic_on' column,
        if applicable, and not already generated by _configure_properties (which is
        typical).

        Also create a setter function which will assign this attribute to the value of
        the 'polymorphic_identity' upon instance construction, also if applicable.  This
    ...
    def _property_from_column(
        ...
        """Generate/update a :class:`.ColumnProperty` given a :class:`_schema.Column` or
        other SQL expression object."""
    ...
    def _post_configure_properties(self) -> None:
        ...
        This is a deferred configuration step which is intended to execute once all
    ...
    def add_property(
        ...
        If the mapper has not been configured yet, just adds the property to the initial
    ...
    def get_property_by_column(self, column: ColumnElement[_T]) -> MapperProperty[_T]:
        """Given a :class:`_schema.Column` object, return the :class:`.MapperProperty`
    ...
    def _mappers_from_spec(
        ...
        Trims the list of mappers to just those represented within the given selectable,
        if present. This helps some more legacy-ish mappings.
        """
        if spec == "*":
    ...
    def _selectable_from_mappers(
        ...
        """Given a list of mappers (assumed to be within this mapper's inheritance
        hierarchy), construct an outerjoin amongst those mapper's mapped tables."""
    ...
    def _has_aliased_polymorphic_fromclause(self):
        ...
        As of #8168, polymorphic adaption with ORMAdapter is used only if this is
        present.
        """
    ...
    def _should_select_with_poly_adapter(self):
        """
        Determine if _MapperEntity or _ORMColumnEntity will need to use polymorphic
        adaption when setting up a SELECT as well as fetching rows for mapped classes
        and subclasses against this Mapper.

        moved here from context.py for #8456 to generalize the ruleset for this
        condition.
        """
    ...
    """The list of :class:`_orm.Mapper` objects included in the default "polymorphic"
    query."""
    ...
    def selectable(self) -> FromClause:
        """
        The :class:`_schema.FromClause` construct this :class:`_orm.Mapper` selects from
        by default.
    ...
    def _iterate_polymorphic_properties(self, mappers=None):
        """Return an iterator of MapperProperty objects which will render into a
        SELECT."""
        if mappers is None:
    ...
    def all_orm_descriptors(self) -> util.ReadOnlyProperties[InspectionAttr]:
        """
        A namespace of all :class:`.InspectionAttr` attributes associated with the
        mapped class.
    ...
    def _pk_synonyms(self) -> Dict[str, str]:
        """Return a dictionary of {syn_attribute_name: pk_attr_name} for all synonyms
        that refer to primary key columns."""
        descriptor_props = util.preloaded.orm_descriptor_props
    ...
    def synonyms(self) -> util.ReadOnlyProperties[SynonymProperty[Any]]:
        """
        Return a namespace of all :class:`.Synonym` properties maintained by this
        :class:`_orm.Mapper`.
    ...
    def column_attrs(self) -> util.ReadOnlyProperties[ColumnProperty[Any]]:
        """
        Return a namespace of all :class:`.ColumnProperty` properties maintained by this
        :class:`_orm.Mapper`.
    ...
    def relationships(
        ...
        """
        A namespace of all :class:`.Relationship` properties maintained by this
        :class:`_orm.Mapper`.
    ...
    def composites(self) -> util.ReadOnlyProperties[CompositeProperty[Any]]:
        """
        Return a namespace of all :class:`.Composite` properties maintained by this
        :class:`_orm.Mapper`.
    ...
    def _get_clause(self):
        """
        Create a "get clause" based on the primary key.

        this is used by query.get() and many-to-one lazyloads to load this item by
        primary key.
        """
        params = [
            (
    ...
    def _equivalent_columns(self) -> _EquivalentColumnMap:
        """
        Create a map of all equivalent columns, based on the determination of column
        pairs that are equated to one another based on inherit condition.  This is
        designed to work with the queries that util.polymorphic_union comes up with,
        which often don't include the columns from the base table directly (including
        the subclass table columns only).
    ...
    def _should_exclude(self, name, assigned_name, local, column):
        """
        Determine whether a particular property should be implicitly present on the
        class.
    ...
    def is_sibling(self, other: Mapper[Any]) -> bool:
        """
        Return true if the other mapper is an inheriting sibling to this one.

        common parent but different branch
        """
        return (
    ...
    def _optimized_get_statement(self, state, attribute_names):
        """
        Assemble a WHERE clause which retrieves a given state by primary key, using a
        minimized set of tables.

        Applies to a joined-table inheritance mapper where the requested attribute names
        are only present on joined tables, not the base table.  The WHERE clause
        attempts to include only those tables to minimize joins.
        """
        props = self._props
    ...
    def _would_selectin_load_only_from_given_mapper(self, super_mapper):
        """
        Return True if this mapper would "selectin" polymorphic load based on the given
        super mapper, and not from a setting from a subclass.
    ...
    def _subclass_load_via_in(self, entity, polymorphic_from):
        """Assemble a that can load the columns local to this subclass as a SELECT with
        IN."""
    ...
    def _table_to_equated(self):
        """Memoized map of tables to collections of columns to be synchronized upwards
        to the base mapper."""
...
def configure_mappers() -> None:
    """
    Initialize the inter-mapper relationships of all mappers that have been constructed
    thus far across all :class:`_orm.registry` collections.
...
def _event_on_init(state, args, kwargs):
    ...
    This also includes mapper compilation, normally not needed here but helps with some
    piecemeal configuration scenarios (such as in the ORM tutorial).
    """
