...
class _AbstractLoad(traversals.GenerativeOnTraversal, LoaderOption):
    ...
    def contains_eager(
        ...
        r"""
        Indicate that the given attribute should be eagerly loaded from columns stated
    ...
    def load_only(self, *attrs: _AttrType, raiseload: bool = False) -> Self:
        r"""
        Indicate that for a particular entity, only the given list of column-based
        attribute names should be loaded; all others will be deferred.
    ...
    def defer(self, key: _AttrType, raiseload: bool = False) -> Self:
        r"""
        Indicate that the given column-oriented attribute should be deferred, e.g. not
    ...
    def options(self, *opts: _AbstractLoad) -> Self:
        r"""
        Apply a series of options as sub-options to this :class:`_orm._AbstractLoad`
        object.
...
class Load(_AbstractLoad):
    ...
    :class:`_sql.Select` or a legacy :class:`_query.Query` in order to affect how
    various mapped attributes are loaded.
    ...
    def _adjust_for_extra_criteria(self, context: QueryContext) -> Load:
        """Apply the current bound parameters in a QueryContext to all occurrences
        "extra_criteria" stored within this ``Load`` object, returning a new instance of
        this ``Load`` object."""
    ...
    def _reconcile_query_entities_with_us(self, mapper_entities, raiseerr):
        """Called at process time to allow adjustment of the root entity inside of
        _LoadElement objects."""
        path = self.path
...
class _LoadElement(
    ...
    """
    Represents strategy information to select for a LoaderStrategy and pass options to
    it.
    ...
    def _adjust_effective_path_for_current_path(
        ...
        """
        Receives the 'current_path' entry from an :class:`.ORMCompileState` instance,
        which is set during lazy loads and secondary loader strategy loads, and adjusts
        the given path to be relative to the current_path.
    ...
    def _prepend_path_from(self, parent: Load) -> _LoadElement:
        """
        Adjust the path of this :class:`._LoadElement` to be a subpath of that of the
        given parent :class:`_orm.Load` object's path.
    ...
    def _reconcile(replacement: _LoadElement, existing: _LoadElement) -> _LoadElement:
        """
        Define behavior for when two Load objects are to be put into the
        context.attributes under the same key.
...
class _AttributeStrategyLoad(_LoadElement):
    ...
    def _generate_extra_criteria(self, context):
        """
        Apply the current bound parameters in a QueryContext to the immediate
        "extra_criteria" stored with this Load object.

        Load objects are typically pulled from the cached version of the statement from
        a QueryContext.  The statement currently being executed will have new values
        (and keys) for bound parameters in the extra criteria which need to be applied
        by loader strategies when they handle this criteria for a result set.
        """
...
class _ClassStrategyLoad(_LoadElement):
    """
    Loader strategies that deals with a class as a target, not an attribute path.
...
def _parse_attr_argument(
    ...
    """
    Parse an attribute or wildcard argument to produce an :class:`._AbstractLoad`
    instance.
...
def loader_unbound_fn(fn: _FN) -> _FN:
    """Decorator that applies docstrings between standalone loader functions and the
    loader methods on :class:`._AbstractLoad`."""
