...
# mypy: ignore-errors
"""Additional ORM persistence classes related to "bulk" operations, specifically outside
...
class ORMDMLState(AbstractORMCompileState):
    ...
    def _setup_orm_returning(
        ...
        *,
        ...
        """
        Establish ORM column handlers for an INSERT, UPDATE, or DELETE which uses
        explicit returning().

        called within compilation level create_for_statement.

        The _return_orm_returning() method then receives the Result after the statement
        was executed, and applies ORM loading to the state that we first established
        here.
        """
...
class BulkUDCompileState(ORMDMLState):
    ...
    def _adjust_for_extra_criteria(cls, global_attributes, ext_info):
        ...
        For all distinct single-table-inheritance mappers represented in the table being
        updated or deleted, produce additional WHERE criteria such that only the
        appropriate subtypes are selected from the total results.
    ...
    def _interpret_returning_rows(cls, result, mapper, rows):
        """
        Return rows that indicate PK cols in mapper.primary_key position for RETURNING
        rows.

        Prior to 2.0.36, this method seemed to be written for some kind of inheritance
        scenario but the scenario was unused for actual joined inheritance, and the
        function instead seemed to perform some kind of partial translation that would
        remove non-PK cols if the PK cols happened to be first in the row, but not
        otherwise.  The joined inheritance walk feature here seems to have never been
        used as it was always skipped by the "local_table" check.

        As of 2.0.36 the function strips away non-PK cols and provides the PK cols for
        the table in mapper PK order.
        """
    ...
    def _do_pre_synchronize_auto(
        ...
        "auto" checks if we can use "evaluate" first, then falls back to "fetch"

        evaluate is vastly more efficient for the common case where session is empty,
        only has a few objects, and the UPDATE statement can potentially match
        thousands/millions of rows.

        OTOH more complex criteria that fails to work with "evaluate" we would hope
        usually correlates with fewer net rows.
        """
...
class BulkORMInsert(ORMDMLState, InsertDMLState):
    ...
    def _setup_for_bulk_insert(self, compiler):
        """
        Establish an INSERT statement within the context of bulk insert.

        This method will be within the "conn.execute()" call that is invoked by
        persistence._emit_insert_statement().
        """
        statement = orm_level_statement = cast(dml.Insert, self.statement)
...
class BulkORMUpdate(BulkUDCompileState, UpdateDMLState):
    ...
    def _setup_for_bulk_update(self, statement, compiler, **kw):
        """
        Establish an UPDATE statement within the context of bulk insert.

        This method will be within the "conn.execute()" call that is invoked by
        persistence._emit_update_statement().
        """
    ...
    def _apply_update_set_values_to_objects(
        ...
        """Apply values to objects derived from an update statement, e.g. UPDATE..SET
        <values>"""
