...
"""
Heuristics related to join conditions as used in :func:`_orm.relationship`.
...
class _RelationshipArg(Generic[_T1, _T2]):
    """Stores a user-defined parameter value that must be resolved and parsed later at
...
class RelationshipProperty(
    ...
    class Comparator(util.MemoizedSlots, PropComparator[_PT]):
        ...
        def __init__(
            ...
            """Construction of :class:`.RelationshipProperty.Comparator` is internal to
            the ORM's attribute mechanics."""
            self.prop = prop
        ...
        def any(
            ...
            **kwargs: Any,
            ...
            """
            Produce an expression that tests a collection against particular criterion,
            using EXISTS.
    ...
    def entity(self) -> _InternalEntityType[_T]:
        """Return the target mapped entity, which is an inspect() of the class or
        aliased class that is referenced by this :class:`.RelationshipProperty`."""
    ...
    def _persists_for(self, mapper: Mapper[Any]) -> bool:
        """Return True if this property will persist values on behalf of the given
        mapper."""
    ...
    def _columns_are_mapped(self, *cols: ColumnElement[Any]) -> bool:
        """Return True if all columns in the given collection are mapped by the tables
        referenced by this :class:`.RelationshipProperty`."""
    ...
    def _generate_backref(self) -> None:
        """Interpret the 'backref' instruction to create a :func:`_orm.relationship`
...
class JoinCondition:
    ...
    def _sanitize_joins(self) -> None:
        """
        Remove the parententity annotation from our join conditions which can leak in
        here based on some declarative patterns and maybe others.
    ...
    def _determine_joins(self) -> None:
        """
        Determine the 'primaryjoin' and 'secondaryjoin' attributes, if not passed to the
        constructor already.

        This is based on analysis of the foreign key relationships between the parent
        and target mapped selectables.
        """
    ...
    def primaryjoin_reverse_remote(self) -> ColumnElement[bool]:
        ...
        If the primaryjoin was delivered here with pre-existing "remote" annotations,
        the local/remote annotations are reversed.  Otherwise, the local/remote
    ...
    def _annotate_fks(self) -> None:
        """Annotate the primaryjoin and secondaryjoin structures with 'foreign'
        annotations marking columns considered as foreign."""
        if self._has_foreign_annotations:
            return
    ...
    def _refers_to_parent_table(self) -> bool:
        """Return True if the join condition contains column comparisons where both
        columns are in both tables."""
    ...
    def _annotate_remote(self) -> None:
        """Annotate the primaryjoin and secondaryjoin structures with 'remote'
        annotations marking columns considered as part of the 'remote' side."""
        if self._has_remote_annotations:
            return
    ...
    def _annotate_remote_secondary(self) -> None:
        """Annotate 'remote' in primaryjoin, secondaryjoin when 'secondary' is
        present."""
    ...
    def _annotate_selfref(
        ...
        """Annotate 'remote' in primaryjoin, secondaryjoin when the relationship is
        detected as self-referential."""
    ...
    def _annotate_remote_from_args(self) -> None:
        """Annotate 'remote' in primaryjoin, secondaryjoin when the 'remote_side' or
        '_local_remote_pairs' arguments are used."""
    ...
    def _annotate_remote_with_overlap(self) -> None:
        """Annotate 'remote' in primaryjoin, secondaryjoin when the parent/child tables
        have some set of tables in common, though is not a fully self-referential
        relationship."""
    ...
    def _annotate_local(self) -> None:
        ...
        This annotates all column elements found simultaneously in the parent table and
        the join condition that don't have a 'remote' annotation set up from
    ...
    def _deannotate_pairs(self, collection: _ColumnPairIterable) -> _MutableColumnPairs:
        """Provide deannotation for the various lists of pairs, so that using them in
        hashes doesn't incur high-overhead __eq__() comparisons against original columns
        mapped."""
    ...
    def join_targets(
        ...
        This takes into account aliasing the join clause to reference the appropriate
        corresponding columns in the target objects, as well as the extra child
        ...
        if extra_criteria:
            ...
            def mark_exclude_cols(
                ...
                """
                Note unrelated columns in the "extra criteria" as either should be
                adapted or not adapted, even though they are not part of our "local" or
                "remote" side.
