...
class AssociationProxyExtensionType(InspectionAttrExtensionType):
    ...
    """
    Symbol indicating an :class:`.InspectionAttr` that's of type
...
class _AssociationProxyProtocol(Protocol[_T]):
    """Describes the interface of :class:`.AssociationProxy` without including
    descriptor methods in the interface."""
...
class AssociationProxyInstance(SQLORMOperations[_T]):
    ...
    Intercepted append/set/assignment events will result in the generation of new
    ...
    def scalar(self) -> bool:
        """Return ``True`` if this :class:`.AssociationProxyInstance` proxies a scalar
...
class AmbiguousAssociationProxyInstance(AssociationProxyInstance[_T]):
    """An :class:`.AssociationProxyInstance` where we cannot determine the type of
    target object."""
...
class _AssociationCollection(Generic[_IT]):
    ...
    Given an associated object, return the 'value'.
    """
    ...
    Given an associated object and a value, store that value on the object.
    """

    lazy_collection: _LazyCollectionProtocol[_IT]
    """A callable returning a list-based collection of entities (usually an object
    attribute managed by a SQLAlchemy relationship())"""
    ...
    def __init__(
        ...
        This will always be a subclass of either _AssociationList, _AssociationSet, or
        _AssociationDict.
        """
...
class _AssociationList(_AssociationSingleItem[_T], MutableSequence[_T]):
    ...
    def __iter__(self) -> Iterator[_T]:
        ...
        For the actual domain objects, iterate over .col instead or just use the
        underlying collection directly from its property on the parent.
        """
