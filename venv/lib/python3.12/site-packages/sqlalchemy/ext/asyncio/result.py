...
class AsyncCommon(FilterResult[_R]):
    ...
    def closed(self) -> bool:
        """
        Proxies the .closed attribute of the underlying result object, if any, else
...
class AsyncResult(_WithKeys, AsyncCommon[Row[_TP]]):
    ...
    def scalars(self, index: _KeyIndexType = 0) -> AsyncScalarResult[Any]:
        """
        Return an :class:`_asyncio.AsyncScalarResult` filtering object which will return
        single elements rather than :class:`_row.Row` objects.
...
class AsyncScalarResult(AsyncCommon[_R]):
    """
    A wrapper for a :class:`_asyncio.AsyncResult` that returns scalar values rather than
...
class AsyncMappingResult(_WithKeys, AsyncCommon[RowMapping]):
    """
    A wrapper for a :class:`_asyncio.AsyncResult` that returns dictionary values rather
    than :class:`_engine.Row` values.
