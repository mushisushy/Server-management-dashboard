...
class ArgSignatureTest(fixtures.TestBase):
    """
    Test that all visit_XYZ() in :class:`_sql.Compiler` subclasses have ``**kw``, for
    #8988.
...
class ExceptionTest(fixtures.TablesTest):
    ...
    DBAPIs vary a lot in exception behavior so to actually anticipate specific
    exceptions from real round trips, we need to be conservative.
    """
...
class IsolationLevelTest(fixtures.TestBase):
    ...
    def test_invalid_level_engine_param(self, testing_engine):
        """Test for the new get_isolation_level_values() method and support for the
        dialect-level 'isolation_level' parameter."""
...
class EscapingTest(fixtures.TestBase):
    ...
    def test_percent_sign_round_trip(self):
        """Test that the DBAPI accommodates for escaped / nonescaped percent signs in a
        way that matches the compiler."""
        m = self.metadata
