...
class _EmptyListener(_InstanceLevelDispatch[_ET]):
    """
    Serves as a proxy interface to the events served by a _ClsLevelDispatch, when there
    are no instance-level events present.
    ...
    def for_modify(self, obj: _DispatchCommon[_ET]) -> _ListenerCollection[_ET]:
        ...
        For _EmptyListener at the instance level of a dispatcher, this generates a new
        _ListenerCollection, applies it to the instance, and returns it.
        """
...
class _CompoundListener(_InstanceLevelDispatch[_ET]):
    ...
    def exec_once(self, *args: Any, **kw: Any) -> None:
        """Execute this event, but only if it has not been executed already for this
        collection."""
    ...
    def exec_once_unless_exception(self, *args: Any, **kw: Any) -> None:
        """
        Execute this event, but only if it has not been executed already for this
        collection, or was called by a previous exec_once_unless_exception call and
        raised an exception.
    ...
    def _exec_w_sync_on_first_run(self, *args: Any, **kw: Any) -> None:
        """
        Execute this event, and use a mutex if it has not been executed already for this
        collection, or was called by a previous _exec_w_sync_on_first_run call and
        raised an exception.
...
class _ListenerCollection(_CompoundListener[_ET]):
    ...
    As of 0.7.9, _ListenerCollection is only first created via the
    _EmptyListener.for_modify() method.
    """
    ...
    def for_modify(self, obj: _DispatchCommon[_ET]) -> _ListenerCollection[_ET]:
        ...
        For _ListenerCollection at the instance level of a dispatcher, this returns
        self.
        """
        return self
