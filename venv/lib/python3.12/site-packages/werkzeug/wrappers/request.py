...
class Request(_SansIORequest):
    """
    Represents an incoming WSGI HTTP request, with headers and body taken from the WSGI
    environment. Has properties and methods for using the functionality defined by
    various HTTP specs. The data in requests object is read-only.
    ...
    def from_values(cls, *args: t.Any, **kwargs: t.Any) -> Request:
        ...
        unittesting, there is a full featured client object (:class:`Client`) that
    ...
    def make_form_data_parser(self) -> FormDataParser:
        """
        Creates the form data parser. Instantiates the :attr:`form_data_parser_class`
    ...
    def _load_form_data(self) -> None:
        ...
        `form` and `files` on the request object to multi dicts filled with the incoming
        form data.  As a matter of fact the input stream will be empty afterwards.  You
        can also call this method to force the parsing of the form data.
    ...
    def data(self) -> bytes:
        """
        The raw data read from :attr:`stream`. Will be empty if the request represents
        form data.
    ...
    def get_data(
        ...
        """
        This reads the buffered incoming data from the client into one bytes object.  By
        default this is cached but that behavior can be changed by setting `cache` to
        `False`.
    ...
    def form(self) -> ImmutableMultiDict[str, str]:
        ...
        function.  This can be changed by setting :attr:`parameter_storage_class` to a
    ...
    def files(self) -> ImmutableMultiDict[str, FileStorage]:
        ...
        files.  Each key in :attr:`files` is the name from the ``<input type="file"
        name="">``.  Each value in :attr:`files` is a Werkzeug
