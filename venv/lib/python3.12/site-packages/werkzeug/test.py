...
def stream_encode_multipart(
    ...
    """
    Encode a dict of values (either strings or file descriptors or :class:`FileStorage`
    objects.) into a multipart encoded string stored in a file descriptor.
...
def encode_multipart(
    ...
    """
    Like `stream_encode_multipart` but returns a tuple in the form (``boundary``,
    ``data``) where data is bytes.
...
def _iter_data(data: t.Mapping[str, t.Any]) -> t.Iterator[tuple[str, t.Any]]:
    """
    Iterate over a mapping that might have a list of values, yielding all key, value
    pairs.

    Almost like iter_multi_items but only allows lists, not tuples, of values so tuples
...
class EnvironBuilder:
    ...
    purposes.  It can be used to quickly create WSGI environments or request objects
    from arbitrary data.
    ...
    def close(self) -> None:
        ...
        If you put real :class:`file` objects into the
...
class ClientRedirectError(Exception):
    """If a redirect loop is detected when using follow_redirects=True with the
    :cls:`Client`, then this exception is raised."""
...
class Client:
    ...
    def delete_cookie(
        ...
        *,
        ...
        """
        Delete a cookie if it exists. Cookies are uniquely identified by ``(domain,
        path, key)``.
    ...
    def _add_cookies_to_wsgi(self, environ: WSGIEnvironment) -> None:
        """
        If cookies are enabled, set the ``Cookie`` header in the environ to the cookies
        that are applicable to the request host and path.
    ...
    def resolve_redirect(
        ...
        """
        Perform a new request to the location given by the redirect response to the
        previous request.
...
def create_environ(*args: t.Any, **kwargs: t.Any) -> WSGIEnvironment:
    ...
    should be the path of the request which defaults to '/'.  The second one can either
    be an absolute path (in that case the host is localhost:80) or a full path to the
    request with scheme, netloc port and the path to the script.
...
def run_wsgi_app(
    ...
    This works best if you pass it an application that returns an iterator all the time.
...
class TestResponse(Response):
    """
    :class:`~werkzeug.wrappers.Response` subclass that provides extra information about
    requests made with the test :class:`Client`.
    ...
    request: Request
    """A request object with the environ used to make the request that resulted in this
    response."""

    history: tuple[TestResponse, ...]
    """
    A list of intermediate responses.

    Populated when the test request
...
class Cookie:
    ...
    max_age: int | None
    """The number of seconds from when the cookie was set at which it is no longer
    valid."""
    ...
    origin_only: bool
    """
    Whether the cookie will be sent for exact domain matches only.
