"""
Implements a number of Python exceptions which can be raised from within a view to
trigger a standard HTTP non-200 response.
...
class HTTPException(Exception):
    ...
    application to render a default error page or you can catch the subclasses of it
    independently and render nicer error messages.
...
class BadRequest(HTTPException):
    ...
    *400* `Bad Request`
    ...
    Raise if the browser sends something to the application the application or server
    cannot handle.
    """
...
class BadRequestKeyError(BadRequest, KeyError):
    """
    An exception that is used to signal both a :exc:`KeyError` and a :exc:`BadRequest`.

    Used by many of the datastructures.
    """
...
class ClientDisconnected(BadRequest):
    """
    Internal exception that is raised if Werkzeug detects a disconnected client.  Since
    the client is already gone at that point attempting to send the error message to the
    client might not work and might ultimately result in another exception in the
    server.  Mainly this is here so that it is silenced by default as far as Werkzeug is
    concerned.
...
class SecurityError(BadRequest):
    """
    Raised if something triggers a security error.  This is otherwise exactly like a bad
    request error.
...
class Forbidden(HTTPException):
    ...
    *403* `Forbidden`
    ...
    Raise if the user doesn't have the permission for the requested resource but was
    authenticated.
    """
...
class UnsupportedMediaType(HTTPException):
    ...
    *415* `Unsupported Media Type`
    ...
    The status code returned if the server is unable to handle the media type the client
    transmitted.
    """
...
class FailedDependency(HTTPException):
    ...
    *424* `Failed Dependency`
    ...
    Used if the method could not be performed on the resource because the requested
...
class PreconditionRequired(HTTPException):
    ...
    *428* `Precondition Required`
    ...
    The server requires this request to be conditional, typically to prevent the lost
    update problem, which is a race condition between two or more clients attempting to
    update a resource through PUT or DELETE. By requiring each client to include a
    conditional header ("If-Match" or "If-Unmodified- Since") with the proper value
    retained from a recent GET request, the server ensures that each client has at least
    seen the previous revision of the resource.
    """
...
class _RetryAfter(HTTPException):
    """
    Adds an optional ``retry_after`` parameter which will set the ``Retry-After``
    header.

    May be an :class:`int` number of seconds or
...
class RequestHeaderFieldsTooLarge(HTTPException):
    ...
    *431* `Request Header Fields Too Large`
    ...
    One or more individual fields may be too large, or the set of all headers is too
    large.
    """
...
class UnavailableForLegalReasons(HTTPException):
    ...
    *451* `Unavailable For Legal Reasons`
    ...
    This status code indicates that the server is denying access to the resource as a
...
class BadGateway(HTTPException):
    ...
    *502* `Bad Gateway`
    ...
    If you do proxying in your application you should return this status code if you
    received an invalid response from the upstream server it accessed in attempting to
    fulfill the request.
    """
...
class Aborter:
    """
    When passed a dict of code -> exception items it can be used as callable that raises
    exceptions.  If the first argument to the callable is an integer it will be looked
    up in the mapping, if it's a WSGI application it will be raised in a proxy
    exception.
